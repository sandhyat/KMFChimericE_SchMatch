...
def Train_cross_AE(df_train_preproc, df_rename_preproc, index_for_mapping_orig_to_rename, index_for_mapping_rename_to_orig, reordered_column_names_orig, reordered_column_names_r, mapped_features, Cor_from_df,  Df_holdout_orig, DF_holdout_r,normalizing_values_orig,normalizing_values_r,P_x1):
    ...
    # exit()
    return o_to_r_sim, r_to_o_sim, MisF_X1_te, MisF_X2_te, temp_inf_x1, temp_inf_x2, F1_fromx1, F1_fromx2, overall_quality_error_matching_only, overall_quality_oracle_comb

# def Train_CL0(df_train_preproc, df_rename_preproc, index_for_mapping_orig_to_rename, index_for_mapping_rename_to_orig, reordered_column_names_orig, reordered_column_names_r, mapped_features, Cor_from_df,  Df_holdout_orig, DF_holdout_r,normalizing_values_orig,normalizing_values_r,P_x1):
#     mpfeatures = len(mapped_features)
#     device = torch.device('cuda')
#     num_NonCat_features_orig = len(reordered_column_names_r) - 1
#
...
#
#     dataset_orig = TabularDataset(data=df_train_preproc, output_col=outcome)
#     train_loader_orig = DataLoader(dataset_orig, batch_size_CL, shuffle=True, num_workers=1)
#
#     dataset_r = TabularDataset(data=df_rename_preproc, output_col=outcome)
#     train_loader_r = DataLoader(dataset_r, batch_size_CL, shuffle=True, num_workers=1)
#
...
#     # create an optimizer object
#     # Adam optimizer with learning rate 1e-3
#     optimizer_known = optim.Adam(known_features_encoder.parameters(), lr=learning_rate_CL, weight_decay=1e-5)
#     optimizer_unk_orig = optim.Adam(unknown_features_encoder_orig.parameters(), lr=learning_rate_CL, weight_decay=1e-5)
#     optimizer_unk_r = optim.Adam(unknown_features_encoder_r.parameters(), lr=learning_rate_CL, weight_decay=1e-5)
#
#     # lr scheduler
#     scheduler_known = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_known, patience=2, verbose=True)
#     scheduler_unk_orig = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_unk_orig, patience=2, verbose=True)
#     scheduler_unk_r = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_unk_r, patience=2, verbose=True)
#
#     total_loss = []
...
#                 known_rep_o = known_features_encoder(x_kn_o)
#                 known_rep_r = known_features_encoder(x_kn_r)
#                 unknown_rep_o = unknown_features_encoder_orig(x_unkn_o)
#                 unknown_rep_r = unknown_features_encoder_r(x_unkn_r)
#
#                 # breakpoint()
#                 # embeddings from known features of both databases followed by the unknown features
#                 contrastive_loss = NTXentLoss(known_rep_o, known_rep_r, unknown_rep_o, unknown_rep_r, tau)
#
#                 # compute accumulated gradients
...
#
#                 # add the mini-batch training loss to epoch loss
#                 loss_tr += contrastive_loss.item()
#
#         # compute the epoch training loss
#         loss_tr = loss_tr / (len(train_loader_orig) + len(train_loader_r))
#
#         # TODO validation los with the scheduler
#
#         # display the epoch training loss
#         print("epoch : {}/{}, total loss = {:.8f}".format(epoch + 1, epochs_CL, loss_tr))
#         # total_loss.append(loss)
#
#     # breakpoint()
#
...
#     grad_sum_unkn_o = torch.zeros((represFinal_dim, temp_input.shape[-1])).to(device)
...
#     grad_sum_unkn_r = torch.zeros((represFinal_dim, temp_input.shape[-1])).to(device)
#     for i in range(temp_input.shape[0]):
...
def Train_CL(df_train_preproc, df_rename_preproc, index_for_mapping_orig_to_rename, index_for_mapping_rename_to_orig,
    ...
    return grad_sum_unkn_o, grad_sum_unkn_r, MisF_X1_te, MisF_X2_te, F1_fromx1, F1_fromx2, rep_known_val_o, rep_known_val_r, overall_quality_error_matching_only

def Train_CL_withDec(df_train_preproc, df_rename_preproc, index_for_mapping_orig_to_rename, index_for_mapping_rename_to_orig,
               reordered_column_names_orig, reordered_column_names_r, mapped_features, Cor_from_df, Df_holdout_orig,
               DF_holdout_r, normalizing_values_orig, normalizing_values_r, P_x1):
    mpfeatures = len(mapped_features)
    device = torch.device('cuda')
...
def Train_KMF(df_train_preproc, df_rename_preproc,index_for_mapping_orig_to_rename,
    ...
    true_match_list = ['Col' + str(i + 1) for i in range(len(reordered_column_names_orig) -1)]
    match_dict = dict(zip(true_match_list, true_match_list))
    ...
    return CorMatrix_X1_unmap_mapped, CorMatrix_X2_unmap_mapped, Mistakes_X1_te_KMF, Mistakes_X2_te_KMF, F1_fromx1, F1_fromx2, overall_quality_error_matching_only

def CL_with_KMF_linear(grad_sum_unkn_o, grad_sum_unkn_r, CorMatrix_X1_unmap_mapped, CorMatrix_X2_unmap_mapped,index_for_mapping_orig_to_rename,
                   index_for_mapping_rename_to_orig
                   , reordered_column_names_orig, reordered_column_names_r,
                   mapped_features,P_x1, Df_holdout_orig):
    mpfeatures = len(mapped_features)
    device = torch.device('cuda')
    num_NonCat_features_orig = len(reordered_column_names_orig) - 1
    ...
    return  MisF_X1_te_CL_KMFl, MisF_X2_te_CL_KMFl, F1_fromx1, F1_fromx2, overall_quality_error_matching_only
...
def CL_with_KMF_CLencoded(grad_sum_unkn_o, grad_sum_unkn_r, rep_known_val_o, rep_known_val_r, Df_holdout_orig, DF_holdout_r,
    ...
    num_features = len(reordered_column_names_orig) - 1
    num_NonCat_features_r = len(reordered_column_names_r) - 1
    ...
    return MisF_X1_te_CL_KMFen, MisF_X2_te_CL_KMFen, F1_fromx1, F1_fromx2, overall_quality_error_matching_only
...
# def NTXentLoss0(embeddings_knw_o, embeddings_knw_r, embeddings_unknw_o, embeddings_unknw_r, temperature = 0.1):  # embeddings from known features of both databases followed by the unknown features
...
def Kang_MI_HC_opt_with_Euclidean_dist(df_train_preproc, df_rename_preproc, true_perm,
    ...
    # exit()
    return MisF_X1_te, F1_fromx1, overall_quality_error_matching_only
...
def RadialGAN(df_train_preproc, df_rename_preproc, index_for_mapping_orig_to_rename, index_for_mapping_rename_to_orig, reordered_column_names_orig, reordered_column_names_r, mapped_features, partition_no, Cor_from_df, Df_holdout_orig,
    ...
    to_map_orig = reordered_column_names_orig[-(num_features - mpfeatures + 1):-1]
    to_map_r = reordered_column_names_r[-(num_features - mpfeatures + 1):-1]
    ...
    true_match_list = ['Col' + str(i + 1) for i in range(num_features)]
    match_dict = dict(zip(true_match_list, true_match_list))
    ...
    # oracle combination metric
    # criterion = nn.MSELoss()
    # overall_quality_oracle_comb = criterion(gen_features_r[:, incorrect_match_idx_r_from_x1], Df_holdout_orig.iloc[:, mpfeatures:-1].values[:,incorrect_match_idx_orig_from_x1])
    # breakpoint()
    return MisF_X1_te, MisF_X2_te, F1_fromx1, F1_fromx2, overall_quality_error_matching_only
...
def main(dataset_no_sample):
    ...
    if dataset_number not in ['Nomao', 'superconductor']:
        ...
        if dataset_number in [1, 2]:
            ...
            filename = "./Syn_data/SD_" + str(dataset_number) + "/2021-05-21Syn_Data_" + str(
                dataset_number) + "_Sample_no_" + str(
    ...
    full_Data1 = full_Data0.copy()
    full_Data1['Y'] = np.where(full_Data1['Y'] == -1, 0, 1)  # to make it compatible with xgbt during evaluation

    full_data_points = full_Data1.shape[0]
    num_features = full_Data1.shape[1] - 1

    # full data initial correlation
    Feature_matrix = full_Data1.iloc[:, :-1]
    ...
    AVG_MISMATCHES_X1_tr_CL = np.zeros((len(list_of_number_samples), n_t))
    AVG_MISMATCHES_X2_tr_CL = np.zeros((len(list_of_number_samples), n_t))

    Frac_mismatches_across_trial_perm_X1_tr_CL = np.zeros((len(list_of_number_samples), n_t * n_p))
    Frac_mismatches_across_trial_perm_X2_tr_CL = np.zeros((len(list_of_number_samples), n_t * n_p))


    AVG_MISMATCHES_X1_tr_CL_with_Dec = np.zeros((len(list_of_number_samples), n_t))
    AVG_MISMATCHES_X2_tr_CL_with_Dec = np.zeros((len(list_of_number_samples), n_t))

    Frac_mismatches_across_trial_perm_X1_tr_CL_with_Dec = np.zeros((len(list_of_number_samples), n_t * n_p))
    Frac_mismatches_across_trial_perm_X2_tr_CL_with_Dec = np.zeros((len(list_of_number_samples), n_t * n_p))


    AVG_MISMATCHES_X1_tr_CL_KMF_l = np.zeros((len(list_of_number_samples), n_t))
    AVG_MISMATCHES_X2_tr_CL_KMF_l = np.zeros((len(list_of_number_samples), n_t))

    Frac_mismatches_across_trial_perm_X1_tr_CL_KMF_l = np.zeros((len(list_of_number_samples), n_t * n_p))
    Frac_mismatches_across_trial_perm_X2_tr_CL_KMF_l = np.zeros((len(list_of_number_samples), n_t * n_p))

    AVG_MISMATCHES_X1_tr_CL_KMF_enc = np.zeros((len(list_of_number_samples), n_t))
    AVG_MISMATCHES_X2_tr_CL_KMF_enc = np.zeros((len(list_of_number_samples), n_t))

    Frac_mismatches_across_trial_perm_X1_tr_CL_KMF_enc = np.zeros((len(list_of_number_samples), n_t * n_p))
    Frac_mismatches_across_trial_perm_X2_tr_CL_KMF_enc = np.zeros((len(list_of_number_samples), n_t * n_p))
    ...
    AVG_F1_X1_tr = np.zeros((len(list_of_number_samples), n_t))
    AVG_F1_X2_tr = np.zeros((len(list_of_number_samples), n_t))

    F1_across_trial_perm_X1_tr = np.zeros((len(list_of_number_samples), n_t * n_p))
    F1_across_trial_perm_X2_tr = np.zeros((len(list_of_number_samples), n_t * n_p))

    AVG_F1_X1_tr_CL = np.zeros((len(list_of_number_samples), n_t))
    AVG_F1_X2_tr_CL = np.zeros((len(list_of_number_samples), n_t))

    F1_across_trial_perm_X1_tr_CL = np.zeros((len(list_of_number_samples), n_t * n_p))
    F1_across_trial_perm_X2_tr_CL = np.zeros((len(list_of_number_samples), n_t * n_p))

    AVG_F1_X1_tr_CL_with_Dec = np.zeros((len(list_of_number_samples), n_t))
    AVG_F1_X2_tr_CL_with_Dec = np.zeros((len(list_of_number_samples), n_t))

    F1_across_trial_perm_X1_tr_CL_with_Dec = np.zeros((len(list_of_number_samples), n_t * n_p))
    F1_across_trial_perm_X2_tr_CL_with_Dec = np.zeros((len(list_of_number_samples), n_t * n_p))

    AVG_F1_X1_tr_CL_KMF_l = np.zeros((len(list_of_number_samples), n_t))
    AVG_F1_X2_tr_CL_KMF_l = np.zeros((len(list_of_number_samples), n_t))

    F1_across_trial_perm_X1_tr_CL_KMF_l = np.zeros((len(list_of_number_samples), n_t * n_p))
    F1_across_trial_perm_X2_tr_CL_KMF_l = np.zeros((len(list_of_number_samples), n_t * n_p))

    AVG_F1_X1_tr_CL_KMF_enc = np.zeros((len(list_of_number_samples), n_t))
    AVG_F1_X2_tr_CL_KMF_enc = np.zeros((len(list_of_number_samples), n_t))

    F1_across_trial_perm_X1_tr_CL_KMF_enc = np.zeros((len(list_of_number_samples), n_t * n_p))
    F1_across_trial_perm_X2_tr_CL_KMF_enc = np.zeros((len(list_of_number_samples), n_t * n_p))

    AVG_F1_X1_tr_sim_Cor = np.zeros((len(list_of_number_samples), n_t))
    AVG_F1_X2_tr_sim_Cor = np.zeros((len(list_of_number_samples), n_t))

    F1_across_trial_perm_X1_tr_sim_Cor = np.zeros((len(list_of_number_samples), n_t * n_p))
    F1_across_trial_perm_X2_tr_sim_Cor = np.zeros((len(list_of_number_samples), n_t * n_p))

    AVG_F1_X1_tr_RG = np.zeros((len(list_of_number_samples), n_t))
    AVG_F1_X2_tr_RG = np.zeros((len(list_of_number_samples), n_t))

    F1_across_trial_perm_X1_tr_RG = np.zeros((len(list_of_number_samples), n_t * n_p))
    F1_across_trial_perm_X2_tr_RG = np.zeros((len(list_of_number_samples), n_t * n_p))

    AVG_F1_X1_tr_Kang = np.zeros((len(list_of_number_samples), n_t))
    AVG_F1_X2_tr_Kang = np.zeros((len(list_of_number_samples), n_t))

    F1_across_trial_perm_X1_tr_Kang = np.zeros((len(list_of_number_samples), n_t * n_p))


    # defining the match mse across trials; storing only for X1 side as that is what we will be using ultimately
    MatchMSE_across_trial_perm_X1_tr = np.zeros((len(list_of_number_samples), n_t * n_p))
    MatchMSE_across_trial_perm_X1_tr_CL = np.zeros((len(list_of_number_samples), n_t * n_p))
    MatchMSE_across_trial_perm_X1_tr_CL_Dec = np.zeros((len(list_of_number_samples), n_t * n_p))
    MatchMSE_across_trial_perm_X1_tr_CL_KMF_l = np.zeros((len(list_of_number_samples), n_t * n_p))
    MatchMSE_across_trial_perm_X1_tr_CL_KMF_en = np.zeros((len(list_of_number_samples), n_t * n_p))
    MatchMSE_across_trial_perm_X1_tr_sim_Cor = np.zeros((len(list_of_number_samples), n_t * n_p))
    MatchMSE_across_trial_perm_X1_tr_Kang = np.zeros((len(list_of_number_samples), n_t * n_p))
    MatchMSE_across_trial_perm_X1_tr_RG = np.zeros((len(list_of_number_samples), n_t * n_p))


    ReconMSE_across_trial_perm_X1_tr = np.zeros((len(list_of_number_samples), n_t * n_p))
    ReconMSE_across_trial_perm_X1_tr_CL_Dec = np.zeros((len(list_of_number_samples), n_t * n_p))
    ...
    for num_samples in list_of_number_samples:
        ...
        full_Data = full_Data1.iloc[sample_index_list].copy()
        ...
        for trial in range(n_t):
            ...
            for partition in range(n_p):
                ...
                Mistakes_X1_te_KANG, F1_x1_kang, match_mse_kang = Kang_MI_HC_opt_with_Euclidean_dist(
                    ...
                    DF_holdout_r,sq_transf_features,P_x1[:-1, :-1])
                # #
                ...
                # #
                Cor_X1_map_unmap, Cor_X2_map_unmap, Mistakes_X1_tr_sim_Cor, Mistakes_X2_tr_sim_Cor, F1_x1_sim_cor, F1_x2_sim_cor,match_mse_kmf = Train_KMF(df_train_preproc.copy(), df_rename_preproc.copy(), index_for_mapping_orig_to_rename, index_for_mapping_rename_to_orig, reordered_column_names_orig, reordered_column_names_r, mapped_features, Cor_from_df,Df_holdout_orig,
                    DF_holdout_r, sq_transf_features,P_x1[len(mapped_features):-1,
                                                                       len(mapped_features):-1])
                # #
                Mistakes_X1_tr_RG, Mistakes_X2_tr_RG, F1_x1_RG, F1_x2_RG, match_mse_RG = RadialGAN(df_train_preproc.copy(), df_rename_preproc.copy(), index_for_mapping_orig_to_rename, index_for_mapping_rename_to_orig, reordered_column_names_orig, reordered_column_names_r, mapped_features ,partition, Cor_from_df, Df_holdout_orig,
                    DF_holdout_r,sq_transf_features,P_x1[len(mapped_features):-1,
                                                                       len(mapped_features):-1])
                # #
                o_to_r_sim_chimeric, r_to_o_sim_chimeric, Mistakes_X1_tr, Mistakes_X2_tr, match_details_x1, match_details_x2, F1_x1, F1_x2, match_mse_chimeric, recon_mse_chimeric = Train_cross_AE(
                    df_train_preproc.copy(), df_rename_preproc.copy(), index_for_mapping_orig_to_rename,
                ...
                #
                _, _, Mistakes_X1_tr_CL, Mistakes_X2_tr_CL, F1_x1_CL, F1_x2_CL, _, _, match_mse_cl = Train_CL(
                    df_train_preproc.copy(), df_rename_preproc.copy(), index_for_mapping_orig_to_rename,
                    index_for_mapping_rename_to_orig, reordered_column_names_orig, reordered_column_names_r,
                    mapped_features, Cor_from_df, Df_holdout_orig,
                    DF_holdout_r, normalizing_values_orig, normalizing_values_r,
                ...
                grad_sum_unkn_o, grad_sum_unkn_r, Mistakes_X1_tr_CL_Dec, Mistakes_X2_tr_CL_Dec, F1_x1_CL_Dec, F1_x2_CL_Dec, rep_known_val_o, rep_known_val_r,  match_mse_cl_dec, recon_mse_cl_dec = Train_CL_withDec(
                    df_train_preproc.copy(), df_rename_preproc.copy(), index_for_mapping_orig_to_rename,
                    index_for_mapping_rename_to_orig, reordered_column_names_orig, reordered_column_names_r,
                    mapped_features, Cor_from_df, Df_holdout_orig,
                    DF_holdout_r, normalizing_values_orig, normalizing_values_r,
                ...
                Mistakes_X1_tr_CL_KMFl, Mistakes_X2_tr_CL_KMFl, F1_x1_CL_CL_KMFl, F1_x2_CL_CL_KMFl, match_mse_cl_kmfl = CL_with_KMF_linear(grad_sum_unkn_o, grad_sum_unkn_r, Cor_X1_map_unmap,
                                   Cor_X2_map_unmap, index_for_mapping_orig_to_rename,
                                   index_for_mapping_rename_to_orig
                                   , reordered_column_names_orig, reordered_column_names_r,
                                   mapped_features, P_x1[len(mapped_features):-1, len(mapped_features):-1], Df_holdout_orig)

                Mistakes_X1_tr_CL_KMFen, Mistakes_X2_tr_CL_KMFen, F1_x1_CL_CL_KMFen, F1_x2_CL_CL_KMFen, match_mse_cl_kmfen = CL_with_KMF_CLencoded(grad_sum_unkn_o, grad_sum_unkn_r, rep_known_val_o, rep_known_val_r,
                ...
                Mistakes_for_fixed_trial_fixed_num_mapped_X1_tr_CL_with_Dec[partition] = len(Mistakes_X1_tr_CL_Dec)/(max_mistakes)
                Mistakes_for_fixed_trial_fixed_num_mapped_X2_tr_CL_with_Dec[partition] = len(Mistakes_X2_tr_CL_Dec)/(max_mistakes)
                ...
                Mistakes_for_fixed_trial_fixed_num_mapped_X1_tr_CL_KMF_enc[partition] = len(Mistakes_X1_tr_CL_KMFen)/max_mistakes
                Mistakes_for_fixed_trial_fixed_num_mapped_X2_tr_CL_KMF_enc[partition] = len(Mistakes_X2_tr_CL_KMFen)/max_mistakes
                ...
                Mistakes_for_fixed_trial_fixed_num_mapped_X1_tr_Kang[partition] = len(Mistakes_X1_te_KANG)/(max_mistakes)
                ...
                # breakpoint()
                F1_across_trial_perm_X1_tr_RG[partition] = F1_x1_RG
                F1_across_trial_perm_X2_tr_RG[partition] = F1_x2_RG

                Frac_mismatches_across_trial_perm_X1_tr_RG[m, run_num] = F1_x1_RG
                Frac_mismatches_across_trial_perm_X2_tr_RG[m, run_num] = F1_x2_RG

                MatchMSE_across_trial_perm_X1_tr_RG[m , run_num] = match_mse_RG
            ...
            print(" Chimeric AE when the number of samples are ", num_samples)
            ...
            print(" Simple_correlation when the number of samples are ", num_samples)
            ...
            print(" CL when the number of samples are ", num_samples)
            ...
            print(" CL + dec when the number of samples are ", num_samples)
            ...
            print(" CL + KMFl when the number of samples are ", num_samples)
            ...
            print(" CL + KMF-enc when the number of samples are ", num_samples)
            ...
            print(" Kang et al's MI and HC based method when the number of samples are ", num_samples)
            ...
            print(" Radial GAN when the number of samples are ", num_samples)
...
parser.add_argument("--mpfeatures", default=15, type=int)  # number of mapped_features
...
# parser.add_argument("--mp_features_steps", default=5, type=float)  # number of mpfeatures to test between start and stop; check compatibility with abbove two
...
parser.add_argument("--dataset_number",  default=5) # could be integer or string
...
list_of_number_samples = [500,1000,2000,5000,10000] #for dataset 5

num_feat_sq_trans = 0

# data details
# outcome = "Y"
# dataset_number = 2 # 2 # 10 is for complex data
# frac_renamed = 0.5
...
if datatype == 'b' :
    saving_dir = './Comparison_'+str(datetime.date.today()) +  "/#ofhidden_layers_" + str(
    num_of_hidden_layers) + '/L_dim_' + str(hidden_dim) + '/Syn_' + str(
    dataset_number) + "_orthoStatus_" + str(orthogonalization_type) + "_BNStatus_" + str(batchnorm) +"_TWO-stage_Num_feat_sq_"+str(num_feat_sq_trans) + "_Alpha_" + str(alpha) + "_PERM_dropout_"+str(dropout_rate)+str(datetime.datetime.now())+"_Binarized"
else:
    saving_dir = './Comparison_'+str(datetime.date.today()) +  "/#ofhidden_layers_" + str(
    num_of_hidden_layers) + '/L_dim_' + str(hidden_dim) + '/Syn_' + str(
    dataset_number) + "_orthoStatus_" + str(orthogonalization_type) + "_BNStatus_" + str(batchnorm) +"_TWO-stage_Num_feat_sq_"+str(num_feat_sq_trans) + "_Alpha_" + str(alpha) + "_PERM_dropout_"+str(dropout_rate)+str(datetime.datetime.now())
if not os.path.exists(saving_dir):
    os.makedirs(saving_dir)
...
AVG_over_Dataset_samples_X1_tr_CL = np.zeros((len(list_of_number_samples), num_of_dataset_samples))
AVG_over_Dataset_samples_X2_tr_CL = np.zeros((len(list_of_number_samples), num_of_dataset_samples))

file_name_CL = saving_dir + "/" + "CL_Mismatch_metric_L_" + str(encKnwMapWidthFinal) + "_Syn_" + str(
    dataset_number) + "_BNStatus_" + str(batchnorm) + "_temp_" + str(tau) + "_.txt"
...
AVG_over_Dataset_samples_X1_tr_CL_with_Dec = np.zeros((len(list_of_number_samples), num_of_dataset_samples))
AVG_over_Dataset_samples_X2_tr_CL_with_Dec = np.zeros((len(list_of_number_samples), num_of_dataset_samples))

file_name_CL_with_Dec = saving_dir + "/" + "CL_with_Dec_Mismatch_metric_L_" + str(encKnwMapWidthFinal) + "_Real_data_BNStatus_" + str(batchnorm) + "_temp_" + str(tau) + "_.txt"
...
AVG_over_Dataset_samples_X1_tr_CL_KMF_l = np.zeros((len(list_of_number_samples), num_of_dataset_samples))
AVG_over_Dataset_samples_X2_tr_CL_KMF_l = np.zeros((len(list_of_number_samples), num_of_dataset_samples))

file_name_CL_KMF_l = saving_dir + "/" + "CL_KMF_l_Mismatch_metric_L_" + str(encKnwMapWidthFinal) + "_Syn_" + str(dataset_number) + "_BNStatus_" + str(batchnorm) + "_temp_" + str(tau) + "_.txt"
...
AVG_over_Dataset_samples_X1_tr_CL_KMF_enc = np.zeros((len(list_of_number_samples), num_of_dataset_samples))
AVG_over_Dataset_samples_X2_tr_CL_KMF_enc = np.zeros((len(list_of_number_samples), num_of_dataset_samples))

file_name_CL_KMF_enc = saving_dir + "/" + "CL_KMF_enc_Mismatch_metric_L_" + str(encKnwMapWidthFinal) + "_Syn_" + str(
    dataset_number) + "_BNStatus_" + str(batchnorm) + "_temp_" + str(tau) + "_.txt"
...
f.write("\n Number of mapped features {0}\t".format(mpfeatures))
f.write("\n")
f.close()
...
file_name_violin = saving_dir + "/" + "F1_For_violin_Mismatch_metric_L_" + str(hidden_dim) + "_Syn_" + str(
    dataset_number) + "_orthoStatus_" + str(orthogonalization_type) + "_BNStatus_" + str(batchnorm) + ".txt"
...
f.write("\n X1_train_RG \n")
f.write("{0}".format(MatchMSE_elongated_X1_tr_list_RG))
f.write("\n X1_train_sim_Cor \n")
...
Mean_over_trials_mismatches_X1_tr_CL = np.zeros(len(list_of_number_samples))
SD_over_trials_mismatches_X1_tr_CL = np.zeros(len(list_of_number_samples))
Mean_over_trials_mismatches_X2_tr_CL = np.zeros(len(list_of_number_samples))
SD_over_trials_mismatches_X2_tr_CL = np.zeros(len(list_of_number_samples))

Mean_over_trials_mismatches_X1_tr_CL_with_Dec = np.zeros(len(list_of_number_samples))
SD_over_trials_mismatches_X1_tr_CL_with_Dec = np.zeros(len(list_of_number_samples))
Mean_over_trials_mismatches_X2_tr_CL_with_Dec = np.zeros(len(list_of_number_samples))
SD_over_trials_mismatches_X2_tr_CL_with_Dec = np.zeros(len(list_of_number_samples))

Mean_over_trials_mismatches_X1_tr_CL_kmfl = np.zeros(len(list_of_number_samples))
SD_over_trials_mismatches_X1_tr_CL_kmfl = np.zeros(len(list_of_number_samples))
Mean_over_trials_mismatches_X2_tr_CL_kmfl = np.zeros(len(list_of_number_samples))
SD_over_trials_mismatches_X2_tr_CL_kmfl = np.zeros(len(list_of_number_samples))

Mean_over_trials_mismatches_X1_tr_CL_kmfen = np.zeros(len(list_of_number_samples))
SD_over_trials_mismatches_X1_tr_CL_kmfen = np.zeros(len(list_of_number_samples))
Mean_over_trials_mismatches_X2_tr_CL_kmfen = np.zeros(len(list_of_number_samples))
SD_over_trials_mismatches_X2_tr_CL_kmfen = np.zeros(len(list_of_number_samples))
...
for i in range(len(list_of_number_samples)):
    ...
    SD_over_trials_mismatches_X1_tr_RG[i] = np.round(np.std(F1_elongated_x1_tr_list_RG[i, :]), decimals=4)
    SD_over_trials_mismatches_X2_tr_RG[i] = np.round(np.std(F1_elongated_x2_tr_list_RG[i, :]), decimals=4)
...
if datatype == 'b':
    plt.title("Binarized Dataset no. " + str(dataset_number) + " Results from X1 cross correlation matrix ")
else:
    plt.title("Dataset no. " + str(dataset_number) + " Results from X1 cross correlation matrix ")
plt.legend()
plt.savefig(saving_dir + "/Comp_Twostage_vsothers_X1_tr_Syn_" + str(dataset_number) + "_varyingData_num_Mapped_fea_" + str(
    len(list_of_number_samples)) + "_Num_feat_sq_"+str(num_feat_sq_trans)+str(datatype)+".pdf", bbox='tight')
plt.savefig(saving_dir + "/Comp_Twostage_vsothers_X1_tr_Syn_" + str(dataset_number) + "_varyingData_num_Mapped_fea_" + str(
    len(list_of_number_samples)) + "_Num_feat_sq_"+str(num_feat_sq_trans)+str(datatype)+ ".png", bbox='tight')
plt.close()


plt.scatter(x_axis, Mean_over_trials_mismatches_X2_tr, color='blue', label=" Chimeric AE ", linestyle='None')
...
plt.errorbar(x_axis1, Mean_over_trials_mismatches_X2_tr_sim_Cor, yerr=SD_over_trials_mismatches_X2_tr_sim_Cor, linestyle="solid",
             color='red')
plt.scatter(x_axis, Mean_over_trials_mismatches_X2_tr_CL, color='yellow', label=" CL ", linestyle='None')
plt.errorbar(x_axis, Mean_over_trials_mismatches_X2_tr_CL, yerr=SD_over_trials_mismatches_X2_tr_CL, linestyle="solid",
             color='yellow')
plt.scatter(x_axis, Mean_over_trials_mismatches_X2_tr_CL_kmfl, color='brown', label=" CL + KMFl", linestyle='None')
plt.errorbar(x_axis, Mean_over_trials_mismatches_X2_tr_CL_kmfl, yerr=SD_over_trials_mismatches_X2_tr_CL_kmfl, linestyle="solid",
             color='brown')
plt.scatter(x_axis, Mean_over_trials_mismatches_X2_tr_CL_kmfen, color='green', label=" CL + KMFen", linestyle='None')
plt.errorbar(x_axis, Mean_over_trials_mismatches_X2_tr_CL_kmfen, yerr=SD_over_trials_mismatches_X2_tr_CL_kmfen, linestyle="solid",
             color='green')
...
plt.ylabel("F1 score across different dataset samples")
if datatype == 'b':
    plt.title("Binarized Dataset no. " + str(dataset_number) + " Results from X2 cross correlation matrix ")
else:
    plt.title("Dataset no. " + str(dataset_number) + " Results from X2 cross correlation matrix ")
plt.legend()
plt.savefig(saving_dir + "/Comp_Twostage_vsothers_X2_tr_Syn_" + str(dataset_number) + "_varyingData_num_Mapped_fea_" + str(
    len(list_of_number_samples)) + "_Num_feat_sq_"+str(num_feat_sq_trans) +str(datatype)+".pdf", bbox='tight')
plt.savefig(saving_dir + "/Comp_Twostage_vsothers_X2_tr_Syn_" + str(dataset_number) + "_varyingData_num_Mapped_fea_" + str(
    len(list_of_number_samples)) + "_Num_feat_sq_"+str(num_feat_sq_trans) +str(datatype)+".png", bbox='tight')
plt.close()

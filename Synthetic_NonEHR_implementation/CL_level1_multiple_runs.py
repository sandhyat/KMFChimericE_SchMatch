...
def Train_CL(df_train_preproc, df_rename_preproc, index_for_mapping_orig_to_rename, index_for_mapping_rename_to_orig,
    ...
    if False:
        ...
        plt.legend()
        plt.savefig(
            saving_dir +  "/Before_training_Comp_ModalityGapSVD_Dataset_" + str(dataset_number) + "X_o_dim_" + str(
    ...
    if False:
        ...
        plt.legend()
        plt.savefig(
            saving_dir + "/After_training_Comp_ModalityGapSVD_Dataset_" + str(dataset_number) + "X_o_dim_" + str(
                encKnwMapWidthFinal) + "_tau_" + str(tau) + "_mapped_features_" + str(mpfeatures) + "_#datapoints_" + str(
                points_to_plot) + "_epochs_" + str(epochs) +".png")
        plt.savefig(
            saving_dir + "/After_training_Comp_ModalityGapSVD_Dataset_" + str(dataset_number) + "X_o_dim_" + str(
...
def main(dataset_no_sample):
    ...
    if dataset_number not in ['Nomao', 'superconductor']:
        ...
        if dataset_number in ['1', '2']:
            filename = data_dir + "Syn_data/SD_" + str(dataset_number) + "/2021-05-18Syn_Data_" + str(
                dataset_number) + "_Sample_no_" + str(
                dataset_no_sample) + "_size_20_10000_for_AE_balanced.csv"  # for dataset 1 and 2
        else:
            filename = data_dir + "Syn_data/SD_" + str(dataset_number) + "/2021-05-21Syn_Data_" + str(
                dataset_number) + "_Sample_no_" + str(
    ...
    MatchMSE_across_trial_perm_X1_tr_RG = np.zeros((len(list_of_number_mapped_variables), n_t * n_p))
...
### the current settings are the best ones for bijective superconductor dataset,

## for the Encoders width (Known mapped)
parser.add_argument("--encKnwMapDepth",  default=5, type=int) #
parser.add_argument("--encKnwMapWidth",  default=49, type=int) #
parser.add_argument("--encKnwMapWidthFinal",  default=17, type=int) # this should be same for all three encoders by design
...
parser.add_argument("--encUknwD1OrigWidth",  default=77, type=int) #
...
parser.add_argument("--encUknwD2ReDepth",  default=6, type=int) #
parser.add_argument("--encUknwD2ReWidth",  default=48, type=int) #
...
## learning parameters
parser.add_argument("--batchSize",  default=44, type=int) #
parser.add_argument("--learningRate",  default=0.0023, type=float) #
parser.add_argument("--learningRateFactor",  default=0.3728, type=float) #
parser.add_argument("--LRPatience",  default=4, type=int) #
parser.add_argument("--epochs",  default=42, type=int) #

## CL specific parameters
parser.add_argument("--tau", default=0.1622, type=float)  # temperature parameter in CL loss
parser.add_argument("--masking_ratio", default=0.3848, type=float)  # ratio for creating a new self augmented view
...
parser.add_argument("--weightDirDecoder", default =0.7873, type=float)
parser.add_argument("--weightCrossDecoder", default =0.4296, type=float)
parser.add_argument("--weightCombDecoder", default =0.1964, type=float)

## dataset and setup parameters
parser.add_argument("--dataset_number",  default='superconductor') # could be integer or string
...
# n_p = 5  # number of permutations
# n_t = 3  # number of data partitioning trials
# # list_of_number_mapped_variables = [10]
# list_of_number_mapped_variables = [7]  # for dataset 1,2
list_of_number_mapped_variables = [2,3, 5, 8, 10, 12]  # for dataset 1,2, Nomao, superconduct (perm)
# list_of_number_mapped_variables = [5,10,15,20,25,30] # for dataset 5 and
# list_of_number_mapped_variables = [15,20,25,30,35,40] # for Nomao, superconduct (partial)
# list_of_number_mapped_variables = [15] # for superconduct
...
if datatype == 'b' :
    saving_dir = output_dir + 'Comparison_'+str(datetime.date.today()) +  "/#ofhidden_layers_" + str(
...
fig_saving_dir = output_dir + 'CL_diag_Figures_' + str(datetime.date.today()) + "/Dataset_" + str(dataset_number)
...
file_name_violin_recon_mse = saving_dir + "/" + "ReconMSE_For_violin_Mismatch_metric_L_" + str(
    hidden_dim) + "_orthoStatus_" + str(orthogonalization_type) + "_BNStatus_" + str(batchnorm) + ".txt"
...
plt.savefig(saving_dir + "/Comp_Twostage_vsothers_X1_tr_Syn_" + str(dataset_number) + "_varyingData_num_Mapped_fea_" + str(
    len(list_of_number_mapped_variables)) + "_Num_feat_sq_"+str(num_feat_sq_trans)+str(datatype)+".pdf", bbox='tight')
plt.savefig(saving_dir + "/Comp_Twostage_vsothers_X1_tr_Syn_" + str(dataset_number) + "_varyingData_num_Mapped_fea_" + str(
    len(list_of_number_mapped_variables)) + "_Num_feat_sq_"+str(num_feat_sq_trans)+str(datatype)+ ".png", bbox='tight')
plt.close()

...
Fraction of mistakes could be misleading as the denominator changes.
However, F score is accurate
...
class AE_CL_withDec_old(nn.Module):
  def __init__(self, **kwargs):
    super().__init__()
  ...
  def forward(self, data, cross=0):
      if cross==0:
          ...
          if (len(self.hidden_layers_dec) > 1):
              for thisindex in range(len(self.hidden_layers_dec)-1):
                  code1 = torch.tanh(self.hidden_layers_dec[thisindex+1](code1))
          return code0, code1
      if cross==1:
          code1 = data
          code1 = torch.tanh(self.hidden_layers_dec[0](code1))
...
def Train_CL(df_train_preproc, df_rename_preproc, P_x1, reordered_column_names_orig, reordered_column_names_r, mapped_features, Cor_from_df,  Df_holdout_orig, DF_holdout_r):
      ...
      with torch.no_grad():
        ...
        for i, data in enumerate(zip(val_loader_orig, val_loader_r)):
          if len(data[0][1]) == len(data[1][1]):
            ...
            # breakpoint()
            # embeddings from known features of both databases followed by the unknown features
            contrastive_loss_val = NTXentLoss(known_rep_o, known_rep_r, unknown_rep_o, unknown_rep_r, tau)
        ...
        print("epoch : {}/{}, training loss = {:.8f}, validation loss = {:.8f}".format(epoch + 1, epochs, loss_tr, loss_val))
    ...
    # exit()
    return grad_sum_unkn_o, grad_sum_unkn_r, correct_with_match_from_x1_test, correct_with_match_from_x2_test, temp_inf_x1, temp_inf_x2, F1_fromx1, F1_fromx2, rep_known_val_o, rep_known_val_r

def Train_CL_withDec(df_train_preproc, df_rename_preproc, P_x1, reordered_column_names_orig, reordered_column_names_r, mapped_features, Cor_from_df,  Df_holdout_orig, DF_holdout_r):
    mpfeatures = len(mapped_features)
    device = torch.device('cuda')
    num_NonCat_features_orig = len(reordered_column_names_orig)
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            if True:
                ...
                # display the epoch training loss
                print("Validation performance epoch : {}/{}, loss = {:.5f}, within cont loss = {:.5f}".format(
                        epoch + 1, epochs, loss_val, within_unkn_CL_loss_val))
            else:
                for param in unknown_features_encoder_orig.parameters(): param.requires_grad = False
                temp_input = torch.Tensor(Df_holdout_orig.iloc[:, mpfeatures:].values).to(device)
...
def Train_KMF(df_train_preproc, df_rename_preproc,
    ...
    return CorMatrix_X1_unmap_mapped, CorMatrix_X2_unmap_mapped, correct_with_match_from_x1_test, correct_with_match_from_x2_test, temp_inf_x1, temp_inf_x2, F1_fromx1, F1_fromx2
...
def CL_with_KMF_linear(grad_sum_unkn_o, grad_sum_unkn_r, CorMatrix_X1_unmap_mapped, CorMatrix_X2_unmap_mapped,P_x1
    ...
    return  correct_with_match_from_x1_te_CL_KMFl, correct_with_match_from_x2_te_CL_KMFl, temp_inf_x1, temp_inf_x2, F1_fromx1, F1_fromx2
...
def CL_with_KMF_CLencoded(grad_sum_unkn_o, grad_sum_unkn_r, rep_known_val_o, rep_known_val_r, Df_holdout_orig, DF_holdout_r,
    ...
    print(" CL KMFen X1_train mistakes (len)", unmapped_features_orig-correct_with_match_from_x1_te_CL_KMFen-num_xtra_feat_inX1, "out of ", unmapped_features_orig-num_xtra_feat_inX1)
    print(" CL KMFen X2_train mistakes (len)", unmapped_features_orig-correct_with_match_from_x2_te_CL_KMFen-num_xtra_feat_inX1, "out of ", unmapped_features_orig-num_xtra_feat_inX1)
    ...
    return correct_with_match_from_x1_te_CL_KMFen, correct_with_match_from_x2_te_CL_KMFen, temp_inf_x1, temp_inf_x2, F1_fromx1, F1_fromx2
...
def Train_cross_AE(df_train_preproc, df_rename_preproc, P_x1, reordered_column_names_orig, reordered_column_names_r,
    ...
    return o_to_r_sim, r_to_o_sim, correct_with_match_from_x1_test, correct_with_match_from_x2_test, temp_inf_x1, temp_inf_x2, F1_fromx1, F1_fromx2
...
def main(dataset_no_sample):
    ...
    for mpfeatures in list_of_number_mapped_variables:
        ...
        for trial in range(n_t):
            ...
            for partition in range(n_p):
                ...
                unmapped_CV = [i for i in df_train_preproc.columns if i not in list_lab_ids] # not moving the premappped labs to the chartevents set
                unmapped_MV = [i for i in df_rename_preproc.columns if i not in list_lab_ids] # not moving the premappped labs to the chartevents set
                ...
                Cor_X1_map_unmap, Cor_X2_map_unmap,correct_with_match_from_x1_test_sim_cor, correct_with_match_from_x2_test_sim_cor, temp_infer_from_x1_sim_cor, temp_infer_from_x2_sim_cor, F1_fromx1_simcor, F1_fromx2_simcor = Train_KMF(df_train_preproc.copy(), df_rename_preproc.copy(), P_x1[len(mapped_features):,len(mapped_features):],
                                    reorder_column_CV, reorder_column_MV, mapped_features, Cor_df, DF_holdout_orig,
                                    Df_holdout_r)



                # o_to_r_sim_chimeric, r_to_o_sim_chimeric,correct_with_match_from_x1_test, correct_with_match_from_x2_test, temp_infer_from_x1, temp_infer_from_x2, F1_fromx1, F1_fromx2 = \
                ...
                _, _, correct_with_match_from_x1_test_CL, correct_with_match_from_x2_test_CL, temp_infer_from_x1_CL, temp_infer_from_x2_CL, F1_x1_CL, F1_x2_CL, rep_known_val_o, rep_known_val_r = Train_CL(
                    df_train_preproc.copy(), df_rename_preproc.copy(), P_x1[len(mapped_features):, len(mapped_features):], reorder_column_CV,  reorder_column_MV, mapped_features, Cor_df, DF_holdout_orig, Df_holdout_r)

                grad_sum_unkn_o, grad_sum_unkn_r, correct_with_match_from_x1_test_CL_withdec, correct_with_match_from_x2_test_CL_withdec, temp_infer_from_x1_CL_withdec, temp_infer_from_x2_CL_withdec, F1_x1_CL_withdec, F1_x2_CL_withdec, rep_known_val_o, rep_known_val_r = Train_CL_withDec(
                    df_train_preproc.copy(), df_rename_preproc.copy(), P_x1[len(mapped_features):, len(mapped_features):], reorder_column_CV,  reorder_column_MV, mapped_features, Cor_df, DF_holdout_orig, Df_holdout_r)

                correct_with_match_from_x1_test_CL_KMFl, correct_with_match_from_x2_test_CL_KMFl, temp_infer_from_x1_CL_KMFl, temp_infer_from_x2_CL_KMFl, F1_x1_CL_CL_KMFl, F1_x2_CL_CL_KMFl = CL_with_KMF_linear(grad_sum_unkn_o, grad_sum_unkn_r, Cor_X1_map_unmap,
                                   Cor_X2_map_unmap, P_x1[len(mapped_features):, len(mapped_features):], reorder_column_CV,  reorder_column_MV,
                                   mapped_features, Cor_df)

                correct_with_match_from_x1_test_CL_KMFen, correct_with_match_from_x2_test_CL_KMFen, temp_infer_from_x1_CL_KMFen, temp_infer_from_x2_CL_KMFen, F1_x1_CL_CL_KMFen, F1_x2_CL_CL_KMFen = CL_with_KMF_CLencoded(grad_sum_unkn_o, grad_sum_unkn_r, rep_known_val_o, rep_known_val_r,
    ...
    #     represFinal_dim) + "L_dim_from_hold_out_sample_CL.csv", index=False)
    ...
    #     represFinal_dim) + "L_dim_from_hold_out_sample_CL_KMFl.csv", index=False)
    ...
    #     represFinal_dim) + "L_dim_from_hold_out_sample_CL_KMFen.csv", index=False)
    # no_match_inference_df_from_x2_CL_KMFen.to_csv(saving_dir + "/" + "Post-hoc_from_x2_" + str(n_t) + "_trials_" + str(
    #     represFinal_dim) + "L_dim_from_hold_out_sample_CL_KMFen.csv", index=False)

    return AVG_F1_X1_tr_sim_Cor, AVG_F1_X2_tr_sim_Cor, np.average(AVG_F1_X1_tr_sim_Cor, axis=1), np.average(AVG_F1_X2_tr_sim_Cor, axis = 1), F1_across_trial_perm_X1_tr_sim_Cor, F1_across_trial_perm_X2_tr_sim_Cor, \
           AVG_F1_X1_tr_CL, AVG_F1_X2_tr_CL, np.average(AVG_F1_X1_tr_CL, axis=1), np.average(AVG_F1_X2_tr_CL, axis = 1), F1_across_trial_perm_X1_tr_CL, F1_across_trial_perm_X2_tr_CL,\
           ...
           AVG_F1_X1_tr_CL_KMF_l, AVG_F1_X2_tr_CL_KMF_l, np.average(AVG_F1_X1_tr_CL_KMF_l, axis=1), np.average(AVG_F1_X2_tr_CL_KMF_l, axis = 1), F1_across_trial_perm_X1_tr_CL_KMF_l, F1_across_trial_perm_X2_tr_CL_KMF_l, \
           AVG_F1_X1_tr_CL_KMF_enc, AVG_F1_X2_tr_CL_KMF_enc, np.average(AVG_F1_X1_tr_CL_KMF_enc, axis=1), np.average(AVG_F1_X2_tr_CL_KMF_enc, axis = 1), F1_across_trial_perm_X1_tr_CL_KMF_enc, F1_across_trial_perm_X2_tr_CL_KMF_enc
...
parser.add_argument("--tau", default=0.2, type=float)  # temperature parameter in CL loss
...
parser.add_argument("--block_stand_comb", default=0, type=int)  # can set thhe type as bool too but for now int it is
parser.add_argument("--dropout_rate_CL", default=0.5, type=float)
...
parser.add_argument("--weightDirDecoder", default =1, type=float)
parser.add_argument("--weightCrossDecoder", default =1, type=float)
parser.add_argument("--weightCombDecoder", default =1, type=float)
...
parser.add_argument("--DischargeSummary", default=0, type=int)
...
if DischargeSummary == 0:
    ...
    list_lab_ids = df_disch_emb_CV.columns



upto_test_idx_CV = int(testDatasetSize * len(full_data_CV0))
test_CV = full_data_CV0.iloc[:upto_test_idx_CV]  # this part of the dataset is not touched during HP tuning
full_data_CV = full_data_CV0.iloc[upto_test_idx_CV:] # this is the dataset which will be divided into two parts for hp tuning

upto_test_idx_MV = int(testDatasetSize * len(full_data_MV0))
test_MV = full_data_MV0.iloc[:upto_test_idx_MV]  # this part of the dataset is not touched during HP tuning
full_data_MV = full_data_MV0.iloc[upto_test_idx_MV:] # this is the dataset which will be divided into two parts for hp tuning

df_train_CV, df_holdout_CV = model_selection.train_test_split(full_data_CV, test_size=0.2,
                                                                    random_state=42)
df_train_MV, df_holdout_MV = model_selection.train_test_split(full_data_MV, test_size=0.2,
                                                                    random_state=42)
...
csvdata = {
  ...
  'Mean_f1_CL_with_Dec': Mean_over_trials_mismatches_X1_tr_CL_with_Dec,

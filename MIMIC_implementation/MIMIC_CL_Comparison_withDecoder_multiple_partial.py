...
this file provides a pipeline of feature mapping problem. The components are:
    ...
    3) Using hospital resident matching algorithm (with capacity 1) on the correlation matrix from 1 and get the final mappings.


Comparison is between CL, Chimeric, KMF, CL + KMFl, CL+KMFen
...
# using this because this is what was used for HP tuning
class AE_CL_withDec_old(nn.Module):
  def __init__(self, **kwargs):
      ...
      self.drop_layer1 = nn.Dropout(
          p=self.drop_out_rate)  # dropout layer just before the input layer to be applied to both views
      self.hidden_layers = torch.nn.ModuleList()
      ## always have at least 1 layer
      self.hidden_layers.append(nn.Linear(in_features=self.input_size, out_features=self.hidden))
      ## sizes for subsequent layers
      hiddensizes = np.ceil(np.linspace(start=self.hidden, stop=self.hidden_final, num=self.hidden_depth)).astype(
          'int64')
      for thisindex in range(len(hiddensizes) - 1):
          self.hidden_layers.append(
              nn.Linear(in_features=hiddensizes[thisindex], out_features=hiddensizes[thisindex + 1]))

      self.hidden_layers_dec = torch.nn.ModuleList()
      hiddenSizes_dec  = np.ceil(np.linspace(start=self.hidden_final, stop=self.input_size, num= self.hidden_depth)).astype('int64')
      # breakpoint()
      for thisindex in range(len(hiddenSizes_dec) - 1):
          self.hidden_layers_dec.append(nn.Linear(in_features=hiddenSizes_dec[thisindex], out_features=hiddenSizes_dec[thisindex + 1]))
  ...
  def forward(self, data, cross=0):
      if cross==0:
          ...
          code1 = code0
          if (len(self.hidden_layers_dec) > 1):
              for thisindex in range(len(self.hidden_layers_dec)):
                  code1 = torch.tanh(self.hidden_layers_dec[thisindex](code1))
          return code0, code1
      if cross==1:
          code1 = data
...
def Train_CL(df_train_preproc, df_rename_preproc, P_x1, reordered_column_names_orig, reordered_column_names_r, mapped_features, Cor_from_df,  Df_holdout_orig, DF_holdout_r, partition, trial):
    ...
    temp_input = torch.Tensor(Df_holdout_orig.iloc[:, mpfeatures:].values).to(device)
    grad_sum_unkn_o = torch.zeros((encKnwMapWidthFinal, temp_input.shape[-1])).to(device)
    for i in range(temp_input.shape[0]): grad_sum_unkn_o += torch.autograd.functional.jacobian(unknown_features_encoder_orig, temp_input[i])
    ...
    temp_input = torch.Tensor(DF_holdout_r.iloc[:, mpfeatures:].values).to(device)
    grad_sum_unkn_r = torch.zeros((encKnwMapWidthFinal, temp_input.shape[-1])).to(device)
    for i in range(temp_input.shape[0]): grad_sum_unkn_r += torch.autograd.functional.jacobian(unknown_features_encoder_r, temp_input[i])
...
def Train_CL_withDec(df_train_preproc, df_rename_preproc, P_x1, reordered_column_names_orig, reordered_column_names_r, mapped_features, Cor_from_df,  Df_holdout_orig, DF_holdout_r, partition, trial):
    ...
    for i in range(len(correct_match_idx_orig_from_x1)): # plotting on the correctly mapped features
        ...
        plt.savefig(saving_dir + "/Cross_recon_qua_"+str(itemid_label_dict[int(al[correct_match_idx_orig_from_x1[i]])]) + "_Cor_Map_" + str(mpfeatures) + "_" + str(trial) + "_" + str(partition) + ".png", bbox='tight')
        plt.close()
...
def main(dataset_no_sample):
    ...
    for mpfeatures in list_of_number_mapped_variables:
        ...
        for trial in range(n_t):
            ...
            max_mistakes = len(df_trainCV.columns) - len(list_lab_ids)  # not moving the premappped labs to the chartevents set
...
parser.add_argument("--encKnwMapWidth",  default=48, type=int) #
parser.add_argument("--encKnwMapWidthFinal",  default=29, type=int) # this should be same for all three encoders by design
...
parser.add_argument("--encUknwD1OrigWidth",  default=69, type=int) #
...
parser.add_argument("--encUknwD2ReWidth",  default=79, type=int) #
...
## learning parameters
parser.add_argument("--batchSize",  default=33, type=int) #
parser.add_argument("--learningRate",  default=0.0022, type=float) #
parser.add_argument("--learningRateFactor",  default=0.3825, type=float) #
parser.add_argument("--LRPatience",  default=2, type=int) #
parser.add_argument("--epochs",  default=45, type=int) #

## CL specific parameters
parser.add_argument("--tau", default=0.7749, type=float)  # temperature parameter in CL loss
parser.add_argument("--masking_ratio", default=0.2348, type=float)  # ratio for creating a new self augmented view
...
parser.add_argument("--dropout_rate_CL", default=0.263, type=float)
...
# list_of_number_mapped_variables = [40]
...
saving_dir = output_dir + 'CL_basedcomparison_'+str(datetime.date.today()) +  '/MIMIC_data/#ofhidden_layers_' + str(
...
# TODO : make the colour pallete more accessible

...
def Train_cross_AE(df_train_preproc, df_rename_preproc, P_x1, reordered_column_names_orig, reordered_column_names_r,
    ...
    return correct_with_match_from_x1_test, correct_with_match_from_x2_test, F1_fromx1, F1_fromx2
...
weight_direct_list =[0.5,0.7,1.0]
weight_cross_list =[0.8,1.0,1.2]
dropout_rate_list = [0.5,0.6]
hidden_dim_list = [20,30]
batch_size_list = [32,64]
learning_rate_list = [1e-2, 1e-3]
...
saving_dir = './ChimericAE_Final/#ofhidden_layers_' + str(num_of_hidden_layers) + "/MIMIC_orthoStatus_" + str(orthogonalization_type) + "_BNStatus_" + str(batchnorm) +"hyperparameter_tuning_roundrobin_updatedF1_metric_GPU" + str(iner_iter)
...
# """ permuting over the known mapped features """
#
# list_lab_ids_chart_updated_CV = list_lab_ids
# list_lab_ids_chart_updated_MV = list_lab_ids
#
...
features_CV = np.random.RandomState(seed=42).permutation(features_CV)
features_MV = np.random.RandomState(seed=42).permutation(features_MV)
...
file_name_fromx1 = saving_dir + "/" + "F1_metric_fromX1_Real_data_orthoStatus_" + str(
    orthogonalization_type) + "_BNStatus_" + str(batchnorm) + "roundrobin.txt"
...
f.write("\n \n MIMIC  data HP tuning F1 values from X1 ")
...
f.write("\n \n *** Chimeric AE Present file settings ***")
f.write("\n \n MIMIC  data HP tuning F1 values from X2 ")
f.write("\n Orthogonalization status {0}\t ".format(orthogonalization_type))
f.write("\n Batch norm {0}\t ".format(batchnorm))
#f.write("\n Size of L {0}\t".format(hidden_dim))
#f.write("\n Weight for direct AE loss {0}\t ".format(weight_direct))
...
F1_holdout_over_Dataset_samples_X1_tr_ump = np.zeros((len(weight_direct_list)*len(weight_cross_list)*len(batch_size_list)*len(dropout_rate_list)*len(hidden_dim_list)*len(learning_rate_list), iner_iter))
F1_holdout_over_Dataset_samples_X2_tr_ump = np.zeros((len(weight_direct_list)*len(weight_cross_list)*len(batch_size_list)*len(dropout_rate_list)*len(hidden_dim_list)*len(learning_rate_list),iner_iter))
...
for weight_direct, weight_cross, batch_size, dropout_rate, hidden_dim, learning_rate in itertools.product(weight_direct_list, weight_cross_list, batch_size_list, dropout_rate_list, hidden_dim_list, learning_rate_list):
    ...
    for num_p in range(iner_iter):

        a = np.random.RandomState(seed=num_p*10).permutation(len(list_lab_ids))

        mapped_for_hp = [list_lab_ids[a] for a in list(a[:int((len(list_lab_ids)/2))])]
        unmapped_for_hp_CV = [col for col in features_CV if
                              (col not in mapped_for_hp) & (col not in onlychart_cont_CV)]
        unmapped_for_hp_MV = [col for col in features_MV if
                              (col not in mapped_for_hp) & (col not in onlychart_cont_MV)]

        unmapped_for_hp_CV = np.random.RandomState(seed=42).permutation(unmapped_for_hp_CV)
        unmapped_for_hp_MV = np.random.RandomState(seed=42).permutation(unmapped_for_hp_MV)

        reordered_column_names_updated_CV = mapped_for_hp + list(unmapped_for_hp_CV)
        reordered_column_names_updated_MV = mapped_for_hp + list(unmapped_for_hp_MV)

        df_train_CV_proc0 = df_train_CV_proc.reindex(columns=reordered_column_names_updated_CV)
        df_holdout_CV_proc0 = df_holdout_CV_proc.reindex(columns=reordered_column_names_updated_CV)
        df_train_MV_proc0 = df_train_MV_proc.reindex(columns=reordered_column_names_updated_MV)
        df_holdout_MV_proc0 = df_holdout_MV_proc.reindex(columns=reordered_column_names_updated_MV)

        # update permutation matrix
        P_x1_updated = np.zeros((len(df_train_CV_proc0.columns), len(df_train_MV_proc0.columns)))

        print("Shape of P_x1 ", P_x1_updated.shape)

        for i in range(len(df_train_CV_proc0.columns)):
            for j in range(len(df_train_MV_proc0.columns)):
                if df_train_CV_proc0.columns[i] == df_train_MV_proc0.columns[j]:
                    P_x1_updated[i,j]=1
                elif df_train_CV_proc0.columns[i] in CV_itemids_with_match:
                    if (match_dic[df_train_CV_proc0.columns[i]]== df_train_MV_proc0.columns[j]):
                        P_x1_updated[i,j]=1
        #
        correct_with_match_from_x1_test, correct_with_match_from_x2_test, F1_fromx1, F1_fromx2 = \
        Train_cross_AE(df_train_CV_proc0.copy(), df_train_MV_proc0.copy(), P_x1_updated[len(mapped_for_hp):,
                                          len(mapped_for_hp):], reordered_column_names_updated_CV, reordered_column_names_updated_MV, mapped_for_hp, df_holdout_CV_proc0, df_holdout_MV_proc0)
        #
        F1_holdout_over_Dataset_samples_X1_tr_ump[ctr,num_p] = F1_fromx1
        F1_holdout_over_Dataset_samples_X2_tr_ump[ctr,num_p] = F1_fromx2
...
print(F1_holdout_over_Dataset_samples_X1_tr_ump)
print(F1_holdout_over_Dataset_samples_X2_tr_ump)

Avg_F1_fromX1 = np.average(F1_holdout_over_Dataset_samples_X1_tr_ump, axis=1)
Avg_F1_fromX2 = np.average(F1_holdout_over_Dataset_samples_X2_tr_ump, axis=1)

print(" Average cross different permutations (X1) ")
print(Avg_F1_fromX1)
print(" Average cross different permutations (X2) ")
print(Avg_F1_fromX2)

idx_from_X1 = np.argmax(Avg_F1_fromX1)
idx_from_X2 = np.argmax(Avg_F1_fromX2)
...
print(" Possible F1 for 2 stage Chimeric (from X1)", Avg_F1_fromX1[idx_from_X1], Avg_F1_fromX2[idx_from_X1])
print(" Possible F1 for 2 stage Chimeric (from X2)", Avg_F1_fromX1[idx_from_X2], Avg_F1_fromX2[idx_from_X2])

# exit()
# plotting
x_axis = np.arange(ctr)

plt.plot(x_axis, Avg_F1_fromX1, label=" # mapped_feature "+str(len(list_lab_ids)/2))
...
plt.legend()
plt.savefig(saving_dir + "/Final_2layer_RoundRobinMapped_Hyper_X1_tr_MIMIC_iner_iter"+ str(iner_iter)+ ".pdf", bbox='tight')
plt.savefig(saving_dir + "/Final_2layer_RoundRobinMapped_Hyper_X1_tr_MIMIC_iner_iter"+ str(iner_iter)+ ".png", bbox='tight')
plt.close()

plt.plot(x_axis, Avg_F1_fromX2, label=" # mapped_feature "+str(len(list_lab_ids)/2))
plt.xticks(x_axis, np.array(hyperparameter_list), rotation = 65, fontsize = 4)
plt.yticks(np.linspace(0,1,11))
plt.xlabel("Different hyperparameter combination")
plt.ylabel("F1 score")
...
plt.legend()
plt.savefig(saving_dir + "/Final_2layer_RoundRobinMapped_Hyper_X2_tr_MIMIC_iner_iter"+ str(iner_iter)+ ".pdf", bbox='tight')
plt.savefig(saving_dir + "/Final_2layer_RoundRobinMapped_Hyper_X2_tr_MIMIC_iner_iter"+ str(iner_iter)+ ".png", bbox='tight')
plt.close()

"""
This code provides a pipeline to feature mapping problem. The two methods that are being compared are Chimeric AE approach and Simple correlation approach.

The components for both the methods consist of :
1) Training Chimeric AE and Simple Correlation on exactly same data
...
def Train_cross_AE(df_train_preproc, df_rename_preproc, index_for_mapping_orig_to_rename, index_for_mapping_rename_to_orig, reordered_column_names_orig, reordered_column_names_r, mapped_features,mapped_features_updated_orig, mapped_features_updated_r, Cor_from_df,  Df_holdout_orig, DF_holdout_r):
    mpfeatures = len(mapped_features)
    device = torch.device('cuda')
...
def main(dataset_no_sample):
    ...
    continuos_features = ['SPL_THEMES', 'RPL_THEMES', 'LVEF', 'Neck', 'PreOp_Diastolic', 'PreOp_Systolic',
                          ...
                          "StopBang_Total"]  # putting StopBang_Total in cont so that it doesn't go into binary
    ...
    for mpfeatures in list_of_number_mapped_variables:
        ...
        for trial in range(n_t):
            ...
            categorical_features_true_order = ["Anesthesia_Type", "VALVULAR_DISEASE","CPAP.Usage", "ASA", "PAP_Type",
                                               "Surg_Type", "FUNCTIONAL_CAPACITY", "SEX", "RACE"]
            ...
            categorical_features = categorical_features_true_order.copy()
            random.shuffle(categorical_features)
            ...
            for i in list(encoded_var):
                if full_data_ohe[i].value_counts()[0] < 5 or full_data_ohe[i].value_counts()[1] < 5:
                    ...
                    if count < sum([uniqu_ele_cat_values[k] for k in range(temp_counter + 1)]):
                        # print(count, sum([uniqu_ele_cat_values[k] for k in range(temp_counter + 1)]))
                        a[temp_counter] = a[temp_counter] - 1
                    while count > sum([uniqu_ele_cat_values[k] for k in range(temp_counter + 1)]):
                        temp_counter = temp_counter + 1
                        # print(count, temp_counter, uniqu_ele_cat_values[temp_counter], sum([uniqu_ele_cat_values[k] for k in range(temp_counter + 1)]))
                        # print("IN the loop")
                        if count < sum([uniqu_ele_cat_values[k] for k in range(temp_counter + 1)]):
                            # print(count, sum([uniqu_ele_cat_values[k] for k in range(temp_counter + 1)]))
                            a[temp_counter] = a[temp_counter] - 1
            ...
            # print(possible_feat_to_shuffle)
            ...
            for partition in range(n_p):
                ...
                Mistakes_X1_tr_RG, Mistakes_X2_tr_RG = RadialGAN(df_train_preproc.copy(), df_rename_preproc.copy(), index_for_mapping_orig_to_rename, index_for_mapping_rename_to_orig, df_train_preproc.columns, reordered_column_names_r, mapped_features ,partition,Cor_from_df,Df_holdout_orig,
                    DF_holdout_r)


                Mistakes_X1_tr_sim_Cor, Mistakes_X2_tr_sim_Cor, match_details_x1_sim_cor, match_details_x2_sim_cor, mapp_fea_to_add, mapp_fea_to_add_match  = Simple_maximum_sim_viaCorrelation(df_train_preproc.copy(), df_rename_preproc.copy(), index_for_mapping_orig_to_rename, index_for_mapping_rename_to_orig, df_train_preproc.columns, reordered_column_names_r, mapped_features, Cor_from_df,Df_holdout_orig,
                    DF_holdout_r)
...
list_of_number_mapped_variables = [2, 3, 4, 5, 7, 9]
# list_of_number_mapped_variables = [4]
# list_of_number_mapped_variables = [2,4,5,6,8,10,12]
#
...
file_name_violin = saving_dir + "/" + "For_violin_Mismatch_metric_L_" + str(
...
plt.legend()
plt.savefig(saving_dir + "/Comp_ChimVsRG_X1_tr_Real_data_varyingData_num_Mapped_fea_" + str(
...
plt.title(" ACTFAST data")
plt.legend()
plt.savefig(saving_dir + "/Comp_ChimVsRG_X2_tr_Real_data_varyingData_num_Mapped_fea_" + str(
    len(list_of_number_mapped_variables)) + ".pdf", bbox='tight')
plt.savefig(saving_dir + "/Comp_ChimVsRG_X2_tr_Real_data_varyingData_num_Mapped_fea_" + str(
    len(list_of_number_mapped_variables)) + ".png", bbox='tight')
plt.close()

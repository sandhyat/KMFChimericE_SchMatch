"""
This code provides a pipeline to feature mapping problem. It compares the KMF (Sim correlation),  two stage procedure (KMF + Sim correlation), Kang and RadialGAN binarized case.
...
3) Also, for KMF -> Chimeric AE , record the cross corrrelation values of the final mappings.
...
def weights_init(size):
    ...
    return Variable(torch.randn(*size, device="cpu") * xavier_stddev, requires_grad=True)
...
def Kang_MI_HC_opt_with_Euclidean_dist(df_train_preproc, df_rename_preproc, true_perm,
                                       ...
                                       mapped_features, Cor_from_df, Df_holdout_orig, DF_holdout_r, sq_transf_features,
                                       P_x1):
    ...
    initial_perm = np.array(list(np.arange(mpfeatures)) + list(
        np.random.choice(np.arange(unmapped_features_r), unmapped_features_orig, replace=False) + mpfeatures))
    ...
    for i in range(len(MI_orig)):
        for j in range(len(MI_orig)):
            ...
            temp = (MI_orig[i, j] - MI_r[initial_perm[i], initial_perm[j]]) * (
                        MI_orig[i, j] - MI_r[initial_perm[i], initial_perm[j]])
    ...
    for iter in range(num_iter):
        ...
        idx_unmp_feature = np.random.choice(np.arange(unmapped_features_orig),
                                            1) + mpfeatures  # random index chosen for swapping the value
        # print(unm_feat)
        ...
        for i in range(len(MI_orig)):
            for j in range(len(MI_orig)):
                ...
                temp0 = (MI_orig[i, j] - MI_r[temp_per[i], temp_per[j]]) * (
                            MI_orig[i, j] - MI_r[temp_per[i], temp_per[j]])
    ...
    print(" KANG  X1_train mistakes number", unmapped_features_orig - correct_total_fromKANG, "out of ",
          unmapped_features_orig)
...
def Simple_maximum_sim_viaCorrelation(df_train_preproc, df_rename_preproc, index_for_mapping_orig_to_rename,
    ...
    temp_x1_x1 = [temp_inf_x1.ump_feature_in_X1[i] for i in
                  list(temp_inf_x1.index)[:num_additional_mapped_for_next_stage_x1]]
    temp_x1_match = [temp_inf_x1.match_byGS[i] for i in
                     list(temp_inf_x1.index)[:num_additional_mapped_for_next_stage_x1]]
    temp_x2_x1 = [temp_inf_x2.ump_feature_in_X1[i] for i in
                  list(temp_inf_x2.index)[:num_additional_mapped_for_next_stage_x2]]
    temp_x2_match = [temp_inf_x2.match_byGS[i] for i in
                     list(temp_inf_x2.index)[:num_additional_mapped_for_next_stage_x2]]
...
def Train_cross_AE(df_train_preproc, df_rename_preproc, index_for_mapping_orig_to_rename,
                   ...
                   Df_holdout_orig, DF_holdout_r, sq_transf_features, filename_for_saving_tran_quality,
                   normalizing_values_orig, normalizing_values_r, P_x1):
    mpfeatures = len(mapped_features)
    device = torch.device('cpu')
    ...
    if datatype == 'c':
        model_orig = AE_2_hidden_layer(input_shape=num_NonCat_features_orig, batchnorm=batchnorm,
                                       drop_out_p=dropout_rate).to(
            device)
        model_r = AE_2_hidden_layer(input_shape=num_NonCat_features_r, batchnorm=batchnorm, drop_out_p=dropout_rate).to(
            device)
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(zip(train_loader_orig, train_loader_r)):
            if len(data[0][1]) == len(data[1][1]):
                ...
                train_loss = weight_direct * (
                            (1 - frac_renamed) * train_loss_orig + frac_renamed * train_loss_r) + orth_weighting * (
    ...
    Mistakes_X1_tr, Mistakes_X2_tr, x1_match_matrix_test, x2_match_matrix_test = Stable_matching_algorithm(
    ...
    print(" sq transformed features passed in mapped X1",
          list(set(sq_transf_features).intersection(mapped_features_updated_orig)))
    print(" sq transformed features passed in mapped X2",
          list(set(sq_transf_features).intersection(mapped_features_updated_r)))

    print(" Chimeric mistakes on sq transformation X1 ", list(set(sq_transf_features).intersection(MisF_X1_te)))
    print(" Chimeric mistakes on sq transformation X2 ", list(set(sq_transf_features).intersection(MisF_X2_te)))

    print(" Chimeric  X1_train mistakes number on holdout set", len(MisF_X1_te), "out of ",
          num_NonCat_features_orig - len(mapped_features))
    print(" Chimeric  X2_train mistakes number on holdout set", len(MisF_X2_te), "out of ",
          num_NonCat_features_orig - len(mapped_features))
    ...
    return MisF_X1_te, MisF_X2_te, temp_inf_x1, temp_inf_x2, F1_fromx1, F1_fromx2


def RadialGAN(df_train_preproc, df_rename_preproc, index_for_mapping_orig_to_rename, index_for_mapping_rename_to_orig,
              reordered_column_names_orig, reordered_column_names_r, mapped_features, partition_no, Cor_from_df,
              Df_holdout_orig,
              DF_holdout_r, sq_transf_features):
    device = torch.device('cpu')
    num_features = len(reordered_column_names_r) - 1
    ...
    bxh_o = Variable(torch.zeros(num_features, device="cpu"), requires_grad=True)

    Whh2_o = weights_init(size=[num_features, int(num_features / 2)])
    bhh2_o = Variable(torch.zeros(int(num_features / 2), device="cpu"), requires_grad=True)

    Wh2h3_o = weights_init(size=[int(num_features / 2), num_features])
    bh2h3_o = Variable(torch.zeros(num_features, device="cpu"), requires_grad=True)

    Wh3y_o = weights_init(size=[num_features, 1])
    bh3y_o = Variable(torch.zeros(1, device="cpu"), requires_grad=True)

    Wxh_r = weights_init(size=[num_features, num_features])
    bxh_r = Variable(torch.zeros(num_features, device="cpu"), requires_grad=True)

    Whh2_r = weights_init(size=[num_features, int(num_features / 2)])
    bhh2_r = Variable(torch.zeros(int(num_features / 2), device="cpu"), requires_grad=True)

    Wh2h3_r = weights_init(size=[int(num_features / 2), num_features])
    bh2h3_r = Variable(torch.zeros(num_features, device="cpu"), requires_grad=True)

    Wh3y_r = weights_init(size=[num_features, 1])
    bh3y_r = Variable(torch.zeros(1, device="cpu"), requires_grad=True)
    ...
    bxh_o_F = Variable(torch.zeros(num_features, device="cpu"), requires_grad=True)

    Whh2_o_F = weights_init(size=[num_features, int(num_features / 2)])
    bhh2_o_F = Variable(torch.zeros(int(num_features / 2), device="cpu"), requires_grad=True)

    Wh2h3_o_F = weights_init(size=[int(num_features / 2), num_features])
    bh2h3_o_F = Variable(torch.zeros(num_features, device="cpu"), requires_grad=True)

    Wh3z_o_F = weights_init(size=[num_features, num_features])
    bh3z_o_F = Variable(torch.zeros(num_features, device="cpu"), requires_grad=True)

    Wxh_r_F = weights_init(size=[num_features, num_features])
    bxh_r_F = Variable(torch.zeros(num_features, device="cpu"), requires_grad=True)

    Whh2_r_F = weights_init(size=[num_features, int(num_features / 2)])
    bhh2_r_F = Variable(torch.zeros(int(num_features / 2), device="cpu"), requires_grad=True)

    Wh2h3_r_F = weights_init(size=[int(num_features / 2), num_features])
    bh2h3_r_F = Variable(torch.zeros(num_features, device="cpu"), requires_grad=True)

    Wh3z_r_F = weights_init(size=[num_features, num_features])
    bh3z_r_F = Variable(torch.zeros(num_features, device="cpu"), requires_grad=True)
    ...
    bzh_o_G = Variable(torch.zeros(num_features, device="cpu"), requires_grad=True)

    Whh2_o_G = weights_init(size=[num_features, int(num_features / 2)])
    bhh2_o_G = Variable(torch.zeros(int(num_features / 2), device="cpu"), requires_grad=True)

    Wh2h3_o_G = weights_init(size=[int(num_features / 2), num_features])
    bh2h3_o_G = Variable(torch.zeros(num_features, device="cpu"), requires_grad=True)

    Wh3x_o_G = weights_init(size=[num_features, num_features])
    bh3x_o_G = Variable(torch.zeros(num_features, device="cpu"), requires_grad=True)

    Wzh_r_G = weights_init(size=[num_features, num_features])
    bzh_r_G = Variable(torch.zeros(num_features, device="cpu"), requires_grad=True)

    Whh2_r_G = weights_init(size=[num_features, int(num_features / 2)])
    bhh2_r_G = Variable(torch.zeros(int(num_features / 2), device="cpu"), requires_grad=True)

    Wh2h3_r_G = weights_init(size=[int(num_features / 2), num_features])
    bh2h3_r_G = Variable(torch.zeros(num_features, device="cpu"), requires_grad=True)

    Wh3x_r_G = weights_init(size=[num_features, num_features])
    bh3x_r_G = Variable(torch.zeros(num_features, device="cpu"), requires_grad=True)
    ...
    print(" RadialGAN  X1_train mistakes number on holdout set", len(MisF_X1_te), "out of ",
          num_features - len(mapped_features))
    print(" RadialGAN  X2_train mistakes number on holdout set", len(MisF_X2_te), "out of ",
          num_features - len(mapped_features))
...
def main(dataset_no_sample):
    ...
    # filename = "./Syn_data/SD_" + str(dataset_number) + "/2021-05-20Syn_Data_" + str(dataset_number) +  "_Sample_no_" + str(dataset_no_sample) +"_size_20_10000_for_AE_balanced.csv" # for dataset 4
    ...
    no_match_inference_df_from_x1 = pd.DataFrame(
        columns=['ump_feature_in_X1', 'match_byGS', 'true_correlation', 'estimated_cross_corr'])
    no_match_inference_df_from_x2 = pd.DataFrame(
        columns=['ump_feature_in_X1', 'match_byGS', 'true_correlation', 'estimated_cross_corr'])

    no_match_inference_df_from_x1_Sim_cor = pd.DataFrame(
        columns=['ump_feature_in_X1', 'match_byGS', 'true_correlation', 'estimated_cross_corr', 'corr_p_value',
                 'SD_rejects_H0'])
    no_match_inference_df_from_x2_Sim_cor = pd.DataFrame(
        columns=['ump_feature_in_X1', 'match_byGS', 'true_correlation', 'estimated_cross_corr', 'corr_p_value',
                 'SD_rejects_H0'])
    ...
    for mpfeatures in list_of_number_mapped_variables:
        ...
        for trial in range(n_t):
            ...
            for partition in range(n_p):
                ...
                Mistakes_X1_te_KANG, F1_x1_kang = Kang_MI_HC_opt_with_Euclidean_dist(
                ...
                Mistakes_X1_tr_sim_Cor, Mistakes_X2_tr_sim_Cor, match_details_x1_sim_cor, match_details_x2_sim_cor, mapp_fea_to_add, mapp_fea_to_add_match, F1_x1_sim_cor, F1_x2_sim_cor = Simple_maximum_sim_viaCorrelation(
                    df_train_preproc.copy(), df_rename_preproc.copy(), index_for_mapping_orig_to_rename,
                    index_for_mapping_rename_to_orig, reordered_column_names_orig, reordered_column_names_r,
                    mapped_features, Cor_from_df, Df_holdout_orig,
                    DF_holdout_r, sq_transf_features, P_x1[len(mapped_features):-1, len(mapped_features):-1])

                # Mistakes_X1_tr_RG, Mistakes_X2_tr_RG = RadialGAN(df_train_preproc.copy(), df_rename_preproc.copy(), index_for_mapping_orig_to_rename, index_for_mapping_rename_to_orig, reordered_column_names_orig, reordered_column_names_r, mapped_features ,partition, Cor_from_df, Df_holdout_orig,
                #   DF_holdout_r,sq_transf_features)
                ...
                P_x1_updated = np.zeros(
                    (len(reordered_column_names_orig_updated), len(reordered_column_names_r_updated)))
                ...
                Mistakes_X1_tr, Mistakes_X2_tr, match_details_x1, match_details_x2, F1_x1, F1_x2 = Train_cross_AE(
                    ...
                    mapped_features_updated_r, Cor_from_df, Df_holdout_orig, DF_holdout_r, sq_transf_features,
                    filename_for_saving_tran_quality, normalizing_values_orig, normalizing_values_r,
                    P_x1_updated[len(mapped_features):-1, len(mapped_features):-1])
    ...
    return AVG_F1_X1_tr, AVG_F1_X2_tr, np.average(AVG_F1_X1_tr, axis=1), np.average(AVG_F1_X2_tr,axis=1), F1_across_trial_perm_X1_tr, F1_across_trial_perm_X2_tr, AVG_F1_X1_tr_sim_Cor, AVG_F1_X2_tr_sim_Cor, np.average(
        AVG_F1_X1_tr_sim_Cor, axis=1), np.average(AVG_F1_X2_tr_sim_Cor, axis=1), F1_across_trial_perm_X1_tr_sim_Cor, F1_across_trial_perm_X2_tr_sim_Cor, AVG_F1_X1_tr_Kang, np.average(
        AVG_F1_X1_tr_Kang, axis=1), F1_across_trial_perm_X1_tr_Kang, AVG_F1_X1_tr_RG, AVG_F1_X2_tr_RG, np.average(AVG_F1_X1_tr_RG, axis=1), np.average(AVG_F1_X2_tr_RG, axis=1), F1_across_trial_perm_X1_tr_RG, F1_across_trial_perm_X2_tr_RG


# AVG_F1_X1_tr, AVG_F1_X2_tr, np.average(AVG_F1_X1_tr, axis=1), np.average(AVG_F1_X2_tr, axis = 1), F1_across_trial_perm_X1_tr, F1_across_trial_perm_X2_tr, AVG_F1_X1_tr_sim_Cor, AVG_F1_X2_tr_sim_Cor, np.average(AVG_F1_X1_tr_sim_Cor, axis=1), np.average(AVG_F1_X2_tr_sim_Cor, axis = 1), F1_across_trial_perm_X1_tr_sim_Cor, F1_across_trial_perm_X2_tr_sim_Cor, AVG_F1_X1_tr_Kang, np.average(AVG_F1_X1_tr_Kang,axis=1), F1_across_trial_perm_X1_tr_Kang, AVG_F1_X1_tr_RG, AVG_F1_X2_tr_RG, np.average(AVG_F1_X1_tr_RG, axis=1), np.average(AVG_F1_X2_tr_RG, axis = 1), F1_across_trial_perm_X1_tr_RG, F1_across_trial_perm_X2_tr_RG


n_p = 3  # number of permutations
n_t = 4  # number of data partitioning trials
# list_of_number_mapped_variables = [2,3,4]
list_of_number_mapped_variables = [2, 3, 4, 6, 8, 10]
num_feat_sq_trans = 0
...
if datatype == 'b':
    saving_dir = './ChimericAE_Final_' + str(datetime.date.today()) + "/#ofhidden_layers_" + str(
        ...
        dataset_number) + "_orthoStatus_" + str(orthogonalization_type) + "_BNStatus_" + str(
        batchnorm) + "_TWO-stage_Num_feat_sq_" + str(num_feat_sq_trans) + "_Alpha_" + str(
        alpha) + "_PERM_dropout_" + str(dropout_rate) + str(datetime.datetime.now()) + "_Binarized"
else:
    saving_dir = './ChimericAE_Final_' + str(datetime.date.today()) + "/#ofhidden_layers_" + str(
        num_of_hidden_layers) + '/L_dim_' + str(hidden_dim) + '/Syn_' + str(
        dataset_number) + "_orthoStatus_" + str(orthogonalization_type) + "_BNStatus_" + str(
        batchnorm) + "_TWO-stage_Num_feat_sq_" + str(num_feat_sq_trans) + "_Alpha_" + str(
        alpha) + "_PERM_dropout_" + str(dropout_rate) + str(datetime.datetime.now())
if not os.path.exists(saving_dir):
    os.makedirs(saving_dir)
...
if os.path.exists(
        file_name_RG):  # removing the existing file so that it doesn't append the file from previous experiments
...
if os.path.exists(
        file_name_violin):  # removing the existing file so that it doesn't append the file from previous experiments
...
for i in range(len(list_of_number_mapped_variables)):
    ...
    Mean_over_trials_mismatches_X1_tr_sim_Cor[i] = np.round(np.mean(AVG_over_Dataset_samples_X1_tr_sim_Cor[i, :]),
                                                            decimals=3)
    Mean_over_trials_mismatches_X2_tr_sim_Cor[i] = np.round(np.mean(AVG_over_Dataset_samples_X2_tr_sim_Cor[i, :]),
                                                            decimals=3)
...
plt.errorbar(x_axis, Mean_over_trials_mismatches_X1_tr, yerr=SD_over_trials_mismatches_X1_tr, linestyle="solid",
             color='blue')
plt.scatter(x_axis1, Mean_over_trials_mismatches_X1_tr_sim_Cor, color='red', label=" Simple_correlation ",
            linestyle='None')
plt.errorbar(x_axis1, Mean_over_trials_mismatches_X1_tr_sim_Cor, yerr=SD_over_trials_mismatches_X1_tr_sim_Cor,
             linestyle="solid",
             color='red')
plt.scatter(x_axis2, Mean_over_trials_mismatches_X1_tr_KANG, color='brown', label=" KANG ", linestyle='None')
plt.errorbar(x_axis2, Mean_over_trials_mismatches_X1_tr_KANG, yerr=SD_over_trials_mismatches_X1_tr_KANG,
             linestyle="solid",
             color='brown')
...
plt.legend()
plt.savefig(
    saving_dir + "/F1_Comp_Twostage_vsothers_X1_tr_Syn_" + str(dataset_number) + "_varyingData_num_Mapped_fea_" + str(
        len(list_of_number_mapped_variables)) + "_Num_feat_sq_" + str(num_feat_sq_trans) + str(datatype) + ".pdf",
    bbox='tight')
plt.savefig(
    saving_dir + "/F1_Comp_Twostage_vsothers_X1_tr_Syn_" + str(dataset_number) + "_varyingData_num_Mapped_fea_" + str(
        len(list_of_number_mapped_variables)) + "_Num_feat_sq_" + str(num_feat_sq_trans) + str(datatype) + ".png",
    bbox='tight')
plt.close()
...
plt.errorbar(x_axis, Mean_over_trials_mismatches_X2_tr, yerr=SD_over_trials_mismatches_X2_tr, linestyle="solid",
             color='blue')
plt.scatter(x_axis1, Mean_over_trials_mismatches_X2_tr_sim_Cor, color='red', label=" Simple_correlation ",
            linestyle='None')
plt.errorbar(x_axis1, Mean_over_trials_mismatches_X2_tr_sim_Cor, yerr=SD_over_trials_mismatches_X2_tr_sim_Cor,
             linestyle="solid",
             color='red')
plt.scatter(x_axis2, Mean_over_trials_mismatches_X1_tr_KANG, color='brown', label=" KANG ", linestyle='None')
plt.errorbar(x_axis2, Mean_over_trials_mismatches_X1_tr_KANG, yerr=SD_over_trials_mismatches_X1_tr_KANG,
...
plt.legend()
plt.savefig(
    saving_dir + "/F1_Comp_Twostage_vsothers_X2_tr_Syn_" + str(dataset_number) + "_varyingData_num_Mapped_fea_" + str(
        len(list_of_number_mapped_variables)) + "_Num_feat_sq_" + str(num_feat_sq_trans) + str(datatype) + ".pdf",
    bbox='tight')
plt.savefig(
    saving_dir + "/F1_Comp_Twostage_vsothers_X2_tr_Syn_" + str(dataset_number) + "_varyingData_num_Mapped_fea_" + str(

...
def Kang_MI_HC_opt_with_Euclidean_dist(df_train_preproc, df_rename_preproc, true_perm,
    ...
    # mistake on non transformed variables
    MisF_X1_te_non_tran = list(set(MisF_X1_te)-set(sq_transf_features))
    print("  Mistakes on non-transformed features X1 ", MisF_X1_te_non_tran)
    ...
    return MisF_X1_te_non_tran, F1_fromx1
...
def Simple_maximum_sim_viaCorrelation(df_train_preproc, df_rename_preproc,index_for_mapping_orig_to_rename,
    ...
    MisF_X1_te_non_tran = list(set(MisF_X1_te)-set(sq_transf_features))
    print(" Mistakes on non-transformed features X1 ", MisF_X1_te_non_tran)
    MisF_X2_te_non_tran = list(set(MisF_X2_te)-set(sq_transf_features))
    print(" Mistakes on non-transformed features X2 ",MisF_X2_te_non_tran)
    ...
    return MisF_X1_te_non_tran, MisF_X2_te_non_tran, temp_inf_x1, temp_inf_x2, final_additional_mapped, final_additional_mapped_corr_match, F1_fromx1, F1_fromx2

def Train_cross_AE(df_train_preproc, df_rename_preproc, index_for_mapping_orig_to_rename, index_for_mapping_rename_to_orig, reordered_column_names_orig, reordered_column_names_r, mapped_features,mapped_features_updated_orig, mapped_features_updated_r, Cor_from_df,  Df_holdout_orig, DF_holdout_r,sq_transf_features,filename_for_saving_tran_quality,normalizing_values_orig,normalizing_values_r,P_x1):
    mpfeatures = len(mapped_features)
    device = torch.device('cuda')
    ...
    print(" sq transformed features passed in mapped X1", list(set(sq_transf_features).intersection(mapped_features_updated_orig)) )
    print(" sq transformed features passed in mapped X2", list(set(sq_transf_features).intersection(mapped_features_updated_r)) )
    ...
    # exit()
    return MisF_X1_te_non_tran, MisF_X2_te_non_tran, temp_inf_x1, temp_inf_x2, F1_fromx1, F1_fromx2
...
def RadialGAN(df_train_preproc, df_rename_preproc, index_for_mapping_orig_to_rename, index_for_mapping_rename_to_orig, reordered_column_names_orig, reordered_column_names_r, mapped_features, partition_no, Cor_from_df, Df_holdout_orig,
                    DF_holdout_r,sq_transf_features,filename_for_saving_tran_quality_RG):
    device = torch.device('cuda')
    num_features = len(reordered_column_names_r) -1
    ...
    # plots for transformation evaluation
    row_index_no_orig = np.random.choice(len(features_true_orig), 500, replace=False)
    row_index_no_rename = np.random.choice(len(features_true_renamed), 500, replace=False)
    if sq_transf_features != []:
        x_b = np.arange(-3, 3.1, 0.1)
        for feature_name in sq_transf_features:
            print('\n ---------- feature name --------- \n ', feature_name)
            col_idxno_orig = reordered_column_names_orig.index(feature_name)
            col_idxno_renamed = reordered_column_names_r.index(feature_name)
            ...
            # sq_orig_feature_value = np.square(denormalizing_orig)
            # x_axis = (sq_orig_feature_value-normalizing_values_r['mean'].loc[feature_name])/normalizing_values_r['std'].loc[feature_name]
            x_axis = features_true_orig[row_index_no_orig, col_idxno_orig]
            y_axis = gen_features_r[row_index_no_orig, col_idxno_renamed]
            ...
            # y_axis = features_reconst_from_crossR_test[row_index_no_orig, col_idxno_renamed] * \
            #          normalizing_values_r['std'].loc[feature_name] + normalizing_values_r['mean'].loc[feature_name]
            plt.plot(x_b, -x_b * x_b, 'r-')
            plt.scatter(x_axis, y_axis, color='blue')
            plt.xlabel("true feature value in X_A")
            plt.ylabel("reconstructed feature value from another encoder of RadialGAN")
            temp = stats.pearsonr(x_axis, y_axis)[0]
            plt.figtext(0.6, 0.8, "Cor_value = " + str(np.round(temp, decimals=3)))
            plt.xlim([min(min(y_axis), min(x_axis)) - 1, max(max(y_axis), max(x_axis)) + 1])
            plt.ylim([min(min(y_axis), min(x_axis)) - 1, max(max(y_axis), max(x_axis)) + 1])
            plt.title(" 2-cluster Gaussian simulated (20-D) \n number of mapped feature  " + str(
                mpfeatures) + " & " + str(feature_name) + " had inverse square trans in X_B while training", fontsize=8)
            plt.savefig(
                filename_for_saving_tran_quality_RG + "_sqfeat_"+str(feature_name) + ".pdf", bbox='tight')
            plt.savefig(
                filename_for_saving_tran_quality_RG + "_sqfeat_"+str(feature_name) + ".png", bbox='tight')
            plt.close()

    del df_rename_preproc
    exit()
    return MisF_X1_te_non_tran, MisF_X2_te_non_tran
    ...
    for mpfeatures in list_of_number_mapped_variables:
        ...
        for trial in range(n_t):
            ...
            if num_feat_sq_trans > 0:
                ...
                #                                         replace=False)
                feat_index_for_trans = [list(df_train_preproc0.columns).index('Col20')]
                for j in feat_index_for_trans:
                    df_train2.iloc[:, j] = -df_train2.iloc[:, j] * df_train2.iloc[:, j] # inverse temp
                    DF_holdout_r0.iloc[:, j] = -DF_holdout_r0.iloc[:, j] * DF_holdout_r0.iloc[:, j] # inverse temp
            ...
            max_mistakes = len(feature_names) - len(mapped_features) - num_feat_sq_trans  # subtracting num of sq features only in the case of sq transformation
            ...
            for partition in range(n_p):
                ...
                filename_for_saving_tran_quality_RG = saving_dir + "/RG_Syn" + str(dataset_number)+"_Transf_quality_sq_part#_" +str(partition)+"_trial#_"+str(trial)+"_#mfeat_"+str(mpfeatures)+"_L_dim_"+str(hidden_dim)
                ...
                filename_for_saving_tran_quality = saving_dir + "/Syn" + str(dataset_number)+"_Transf_quality_sq_part#_" +str(partition)+"_trial#_"+str(trial)+"_#mfeat_"+str(mpfeatures)+"_L_dim_"+str(hidden_dim)
                Mistakes_X1_tr, Mistakes_X2_tr, match_details_x1, match_details_x2, F1_x1, F1_x2 = Train_cross_AE(df_train_preproc.copy(), df_rename_preproc.copy(), index_for_mapping_orig_to_rename_updated, index_for_mapping_rename_to_orig_updated, reordered_column_names_orig_updated, reordered_column_names_r_updated, mapped_features, mapped_features_updated_orig, mapped_features_updated_r, Cor_from_df, Df_holdout_orig, DF_holdout_r,sq_transf_features,filename_for_saving_tran_quality,normalizing_values_orig, normalizing_values_r, P_x1_updated[len(mapped_features):-1,len(mapped_features):-1])
...
#list_of_number_mapped_variables = [6]
...
plt.legend()
plt.savefig(saving_dir + "/F1_Comp_Twostage_vsothers_X2_tr_Syn_" + str(dataset_number) + "_varyingData_num_Mapped_fea_" + str(
    len(list_of_number_mapped_variables)) + "_Num_feat_sq_"+str(num_feat_sq_trans) +str(datatype)+".pdf", bbox='tight')
plt.savefig(saving_dir + "/F1_Comp_Twostage_vsothers_X2_tr_Syn_" + str(dataset_number) + "_varyingData_num_Mapped_fea_" + str(
    len(list_of_number_mapped_variables)) + "_Num_feat_sq_"+str(num_feat_sq_trans) +str(datatype)+".png", bbox='tight')
plt.close()

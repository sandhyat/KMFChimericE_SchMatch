"""
ACTFast dataset where the preops are used as known mapped features and the labs are to be matched between two eras

Note:
    when the columns are constant (especially in KMF), I am assigning them as 0 correlation and the corresponding p values as 1
"""
            ...
            if True:
                ...
                for i, data in enumerate(zip(val_loader_orig, val_loader_r)):
                    if len(data[0][1]) == len(data[1][1]):
                        ...
                        #f = open(file_name_epoch_CL_level, 'a')
                        #f.write("\n \n ***  ***")
                        #f.write("\n \n passed epoch  no.   {0}\t ".format(epoch))
                        #f.write("\n \n when the mapped features are   {0}\t ".format(mpfeatures))
                        #f.write("\n \n")
                        #f.close()
                        if (data[0][1].shape[0] == batchSize):
    ...
    for i in range(x1_match_matrix_test.shape[0]):
        ...
        if np.any(P_x1[i] == 2):
            ...
            temp_inf_x1.loc[i, "Correct_Match"] = sum(temp_inf_x1.loc[i, 'match_byGS'] ==
                                              match_df[
                                                  match_df['ClarityFeature'] == temp_inf_x1.loc[i, 'ump_feature_in_X1']][
                                                  'MV Lab'])
    ...
    for i in range(x2_match_matrix_test.shape[0]):
        ...
        if np.any(P_x1[i] == 2):
            ...
            temp_inf_x2.loc[i, "Correct_Match"] = sum(temp_inf_x2.loc[i, 'match_byGS'] ==
                                              match_df[
                                                  match_df['ClarityFeature'] == temp_inf_x2.loc[i, 'ump_feature_in_X1']][
                                                  'MV Lab'])
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            if True:
                ...
                #f = open(file_name_epoch_CL_Dec_level, 'a')
                #f.write("\n \n ***  ***")
                #f.write("\n \n passed epoch  no.   {0}\t ".format(epoch))
                #f.write("\n \n when the mapped features are   {0}\t ".format(mpfeatures))
                #f.write("\n \n")
...
def Train_KMF(df_train_preproc, df_rename_preproc,
    ...
    # this is being done since there could be some columns in the test data that are constant even though that is not the case in the training set
    ...
    CorMatrix_X1_unmap_mapped_P_value = np.nan_to_num(CorMatrix_X1_unmap_mapped_P_value, nan=1.0)
    CorMatrix_X2_unmap_mapped_P_value = np.nan_to_num(CorMatrix_X2_unmap_mapped_P_value, nan=1.0)
...
def CL_with_KMF_CLencoded(grad_sum_unkn_o, grad_sum_unkn_r, rep_known_val_o, rep_known_val_r, Df_holdout_orig, DF_holdout_r,
    ...
    # this is being done since there could be some columns in the test data that are constant even though that is not the case in the training set
    CorMatrix_X1_unmap_mappedE = np.nan_to_num(CorMatrix_X1_unmap_mappedE)
    CorMatrix_X2_unmap_mappedE = np.nan_to_num(CorMatrix_X2_unmap_mappedE)
...
def Train_cross_AE(df_train_preproc, df_rename_preproc, P_x1, reordered_column_names_orig, reordered_column_names_r,
    ...
    plt.legend()
    plt.savefig('ActFAst_cont_ae_error_Orthtype' +str(orthogonalization_type)+ "_#layers_" +str(num_of_hidden_layers) + "_hid_" +str(hidden_dim)+'.pdf', bbox='tight')
    plt.close()
    ...
    # this is being done since there could be some columns in the test data that are constant even though that is not the case in the training set
    CorMatrix_X1_X1_hat_cross_test = np.nan_to_num(CorMatrix_X1_X1_hat_cross_test)
    CorMatrix_X2_X2_hat_cross_test = np.nan_to_num(CorMatrix_X2_X2_hat_cross_test)
    CorMatrix_X1_X1_hat_cross_P_value_test = np.nan_to_num(CorMatrix_X1_X1_hat_cross_P_value_test, nan=1.0)
    CorMatrix_X2_X2_hat_cross_P_value_test = np.nan_to_num(CorMatrix_X2_X2_hat_cross_P_value_test, nan=1.0)
...
def main(dataset_no_sample):
    ...
    for mpfeatures in list_of_number_mapped_variables:
        ...
        for trial in range(n_t):
            ...
            #f = open(file_name_trial_level, 'a')
            #f.write("\n \n ***  ***")
            #f.write("\n \n passed trial no.   {0}\t ".format(trial))
            #f.write("\t  when the mapped features are   {0}\t ".format(mpfeatures))
            #f.write("\n \n")
            #f.close() 
            ...
            for partition in range(n_p):
                ...
                unmapped_CV = [i for i in df_train_preproc.columns if i not in known_columnslist] # not moving the premappped preops to the labs set
                unmapped_MV = [i for i in df_rename_preproc.columns if i not in known_columnslist] # not moving the premappped preops to the labs set
                ...
                for i in range(len(df_train_preproc.columns)):
                    for j in range(len(df_rename_preproc.columns)):
                        if df_train_preproc.columns[i] == df_rename_preproc.columns[j]:
                            P_x1[i, j] = 1
                        elif df_train_preproc.columns[i] in epic_withmatch:
                            if (match_dic[df_train_preproc.columns[i]] == df_rename_preproc.columns[j]):
                                P_x1[i, j] = 1
                        elif (df_train_preproc.columns[i] in epic_withnomatch) & (
                                df_rename_preproc.columns[j] in mv_withnomatch):
                            P_x1[i, j] = 2
                ...
                #f = open(file_name_part_level, 'a')
                #f.write("\n \n ***  ***")
                #f.write("\n \n passed partition  no.   {0}\t ".format(partition))
                #f.write("\t  when the mapped features are   {0}\t ".format(mpfeatures))
                #f.write("\n \n")
                #f.close()
...
parser.add_argument("--mp_features", default=20, type=int)  # number of mapped_features
...
parser.add_argument("--dataset_number",  default='ACTFAST_preops_vs_Labs') # could be integer or string
...
#file_name_trial_level = output_dir +  "Trial_level.txt"
#if os.path.exists(file_name_trial_level):  # removing the existing file so that it doesn't append the file from previous experiments
#       os.remove(file_name_trial_level)

#file_name_part_level = output_dir +  "Partition_level.txt"
#if os.path.exists(file_name_part_level):  # removing the existing file so that it doesn't append the file from previous experiments
#       os.remove(file_name_part_level)

#file_name_epoch_CL_level = output_dir +  "Epoch_CL_level.txt"
#if os.path.exists(file_name_epoch_CL_level):  # removing the existing file so that it doesn't append the file from previous experiments
#       os.remove(file_name_epoch_CL_level)

#file_name_epoch_CL_Dec_level = output_dir +  "Epoch_CL_Dec_level.txt"
#if os.path.exists(file_name_epoch_CL_Dec_level):  # removing the existing file so that it doesn't append the file from previous experiments
#       os.remove(file_name_epoch_CL_Dec_level)


# reading the data and matching files

mv_labs_train =  pd.read_csv(data_dir+"/mv/mv_train_labs.csv")
mv_labs_test = pd.read_csv(data_dir+"/mv/mv_test_labs.csv")
mv_preops_train =  pd.read_csv(data_dir+"/mv/mv_train_preops.csv")
mv_preops_test = pd.read_csv(data_dir+"/mv/mv_test_preops.csv")
...
epic_labs_train =  pd.read_csv(data_dir+"/epic/epic_train_labs.csv")
epic_labs_test = pd.read_csv(data_dir+"/epic/epic_test_labs.csv")
epic_preops_train =  pd.read_csv(data_dir+"/epic/epic_train_preops.csv")
epic_preops_test = pd.read_csv(data_dir+"/epic/epic_test_preops.csv")
...
# gold standard files
mapped_labs_df = pd.read_csv(data_dir+"/mapped_labs_epic_mv.csv")
mapped_preops_df =  pd.read_csv(data_dir+"/mapped_preops_epic_mv.csv")
...
epic_comb_to_partition = epic_preops_train_proc.join(epic_labs_train)
mv_comb_to_partition = mv_preops_train_proc.join(mv_labs_train)


# only training part of the train set is being used for HP tuning
df_train_CV_proc, df_holdout_CV_proc = model_selection.train_test_split(epic_comb_to_partition, test_size=0.2,
                                                                    random_state=42)
df_train_MV_proc, df_holdout_MV_proc = model_selection.train_test_split(mv_comb_to_partition, test_size=0.2,
                                                                    random_state=42)
...
if False:
    ...
    num_obs_for_cor = min(min(len(df_train_CV_proc), len(df_train_MV_proc)), 1000)

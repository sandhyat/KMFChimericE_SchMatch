...
def Train_CL(df_train_preproc, df_rename_preproc, P_x1, reordered_column_names_orig, reordered_column_names_r, mapped_features, Cor_from_df,  Df_holdout_orig, DF_holdout_r, partition, trial):
    ...
    # breakpoint()
    predicted_match_dic_x1 = dict(zip(list(temp_inf_x1[temp_inf_x1['no_match_or_not'] == 0]['ump_feature_in_X1']),list(temp_inf_x1[temp_inf_x1['no_match_or_not'] == 0]['match_byGS'])))

    # this is basically checking what is the closest thing that the feature from the other database is matched to. So this dictionary has features from the x1/orig/smaller database
...
parser = argparse.ArgumentParser(description='parameters for CL')
...
mv_labs_train =  pd.read_csv(data_dir+"/mv/mv_train_labs_Ageabove18.csv")
mv_labs_test = pd.read_csv(data_dir+"/mv/mv_test_labs_Ageabove18.csv")
mv_preops_train =  pd.read_csv(data_dir+"/mv/mv_train_preops_Ageabove18.csv")
mv_preops_test = pd.read_csv(data_dir+"/mv/mv_test_preops_Ageabove18.csv")
...
saving_dir = output_dir + 'Preops_Vs_Labs_CL_basedcomparison_'+str(datetime.date.today()) +  '/ActFast_data/#ofhidden_layers_' + str(
    num_of_hidden_layers) + '/L_dim_' + str(hidden_dim) + "_orthoStatus_" + str(
    orthogonalization_type) + "_BNStatus_" + str(batchnorm) + "_Alpha_" + str(alpha) + "_comp_"+str(datetime.datetime.now())
...
f.write("\n \n ACTFAST preops vs Labs data results  ")
...
plt.ylabel("F1 score averaged across different trials")
plt.title(str(dataset_number) + " data")
plt.legend()

...
def Simple_maximum_sim_viaCorrelation(df_train_preproc, df_rename_preproc, P_x1
    ...
    return correct_with_match_from_x1_test, correct_with_match_from_x2_test, correct_with_no_match_from_CCx1_test, correct_with_no_match_from_CCx2_test, temp_inf_x1, temp_inf_x2, final_additional_mapped, final_additional_mapped_corr_match
...
def Train_cross_AE(df_train_preproc, df_rename_preproc, P_x1, reordered_column_names_orig, reordered_column_names_r, mapped_features,mapped_features_updated_orig, mapped_features_updated_r,Cor_from_df,Df_holdout_orig, DF_holdout_r,P_x1_true,unmapped_features_extra_orig,filename_for_saving_PM_quality,DF_holdout_orig0_not_includedwhiletraining):
    ...
    for feature_name in unmapped_features_extra_orig:
        ...
        if feature_name in mapped_features_updated_orig:
            plt.savefig(
                filename_for_saving_PM_quality + "_PR_curve_GotMappedafter_KMF" + "_NotInOrig_" + str(feature_name) + ".pdf", bbox='tight')
            plt.savefig(
                filename_for_saving_PM_quality + "_PR_curve_GotMappedafter_KMF" + "_NotInOrig_" + str(feature_name) + ".png", bbox='tight')
        else:
            plt.savefig(
    ...
    return correct_with_match_from_x1_test, correct_with_match_from_x2_test, correct_with_no_match_from_CCx1_test, correct_with_no_match_from_CCx2_test, temp_inf_x1, temp_inf_x2
...
def main(dataset_no_sample):
    ...
    for num_feat_idx in range(len(list_of_total_feat_in_D2_minus_mapped[0])):
        ...
        for trial in range(n_t):
            ...
            for partition in range(n_p):
                ...
                correct_with_match_from_x1_test_sim_cor, correct_with_match_from_x2_test_sim_cor, correct_with_no_match_from_CCx1_test_sim_cor, correct_with_no_match_from_CCx2_test_sim_cor, temp_infer_from_x1_sim_cor, temp_infer_from_x2_sim_cor, mapp_fea_to_add, mapp_fea_to_add_match = Simple_maximum_sim_viaCorrelation(
                    df_train_preproc.copy(), df_rename_preproc.copy(), P_x1[len(mapped_features):-1,
                                                                       len(mapped_features):-1],
                    df_train_preproc.columns, reordered_column_names_r, mapped_features, Cor_from_df, Df_holdout_orig,
                    DF_holdout_r)
                ...
                correct_with_match_from_x1_test, correct_with_match_from_x2_test, correct_with_no_match_from_CCx1_test, correct_with_no_match_from_CCx2_test, temp_infer_from_x1, temp_infer_from_x2 = Train_cross_AE(df_train_preproc.copy(), df_rename_preproc.copy(), P_x1_updated[len(mapped_features):-1,
    ...
    return AVG_MISMATCHES_X1_tr, AVG_MISMATCHES_X2_tr, np.average(AVG_MISMATCHES_X1_tr, axis=1), np.average(
        ...
        axis=1), Frac_mismatches_across_trial_perm_X1_tr, Frac_mismatches_across_trial_perm_X2_tr,\
           AVG_MISMATCHES_X1_tr_Sim_cor, AVG_MISMATCHES_X2_tr_Sim_cor, np.average(AVG_MISMATCHES_X1_tr_Sim_cor, axis=1), np.average(
...
for sample_no in range(1,num_of_dataset_samples+1):
    ...
    AVG_MISMATCHES_X1_tr, AVG_MISMATCHES_X2_tr, m_x1, m_x2_tr, Frac_X1_tr, Frac_X2_tr, \
    AVG_MISMATCHES_X1_tr_Sim_cor, AVG_MISMATCHES_X2_tr_Sim_cor, m_x1_Sim_cor, m_x2_tr_Sim_cor, Frac_X1_tr_Sim_cor, Frac_X2_tr_Sim_cor, AVG_MISMATCHES_X1_tr_KANG, m_X1_tr_KANG, Frac_X1_tr_KANG = main(
        sample_no)

    # for simple correlation similarity based method
...
f.write("\n \n List of total features in large database when small database has 50 features \n ")
...
plt.title("ACTFAST data with 50 features in X1 (small, some with no match) \n and varying features in X2 (large)")
plt.legend()
plt.savefig(saving_dir + "/Not_All_matchesASym_Chimeric_Real_data_varyingData_num_total_fea_inX2_" + str(

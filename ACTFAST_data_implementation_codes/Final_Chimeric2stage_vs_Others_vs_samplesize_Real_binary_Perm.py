"""
This code provides a pipeline to feature mapping problem. The two methods that are being compared are Chimeri AE approach and RadialGAN approach.

The components for both the methods consist of :
1) Training Chimeric AE and RadialGAN on exactly same data
...
An array with rows as the number of samples used and columns as the trial number and cell entry as the avg fraction of mismatches for the a fixed trial and the fixed number of mapped variables
...
from sklearn.metrics import roc_auc_score, average_precision_score, mutual_info_score, pairwise
...
# Training function for cross Autoencoder module

def Train_cross_AE(df_train_preproc, df_rename_preproc, index_for_mapping_orig_to_rename, index_for_mapping_rename_to_orig, reordered_column_names_orig, reordered_column_names_r, mapped_features,mapped_features_updated_orig, mapped_features_updated_r, Cor_from_df,  Df_holdout_orig, DF_holdout_r, P_x1):
    mpfeatures = len(mapped_features)
    device = torch.device('cuda')
    ...
    model_orig = AE_binary(input_shape=num_NonCat_features_orig, drop_out_p=0.6).to(device)
    model_r = AE_binary(input_shape=num_NonCat_features_r, drop_out_p=0.6).to(device)
...
def RadialGAN(df_train_preproc, df_rename_preproc, index_for_mapping_orig_to_rename, index_for_mapping_rename_to_orig, reordered_column_names_orig, reordered_column_names_r, mapped_features, partition_no,Cor_from_df, Df_holdout_orig,
    ...
    def G1(Z):
        ...
        x = torch.sigmoid(h_3.mm(Wh3x_o_G) + bh3x_o_G.repeat(h_3.size(0), 1))
    ...
    def G2(Z):
        ...
        x = torch.sigmoid(h_3.mm(Wh3x_r_G) + bh3x_r_G.repeat(h_3.size(0), 1))
...
def main(dataset_no_sample):
    ...
    categorical_features_true_order = ["Anesthesia_Type", "VALVULAR_DISEASE","CPAP.Usage",
                                       "ASA", "PAP_Type", "Surg_Type", "FUNCTIONAL_CAPACITY","SEX","RACE"]
    ...
    for i in range(n_t):
        temp = categorical_features_true_order.copy()
        random.shuffle(temp)
        shuffled_categoricals.append(temp)

    shuffled_categoricals = np.array(shuffled_categoricals)

    full_data_points = len(full_Data0)
    ...
    for num_samples in list_of_number_samples:
        ...
        full_Data1 = full_Data0.iloc[sample_index_list].copy()
        ...
        for trial in range(n_t):
            ...
            bin_features = [c for c in full_Data.columns if c not in [
                outcome] + continuos_features + list(categorical_features) + features_frm_diagncode + features_frm_freetxt]
            ...
            print("Unique cat elements ",uniqu_ele_cat_values)
            ...
            # drop features that have very very skewed distribution
            index_to_drop_cat_var = []
            count = 0
            a = uniqu_ele_cat_values.copy()
            temp_counter = 0
            for i in list(encoded_var):
                if full_data_ohe[i].value_counts()[0] < 5 or full_data_ohe[i].value_counts()[1] < 5:
                    index_to_drop_cat_var.append(count)
                    if count < sum([uniqu_ele_cat_values[k] for k in range(temp_counter + 1)]):
                        a[temp_counter] = a[temp_counter] - 1
                    while count > sum([uniqu_ele_cat_values[k] for k in range(temp_counter + 1)]):
                        temp_counter = temp_counter + 1
                        if count < sum([uniqu_ele_cat_values[k] for k in range(temp_counter + 1)]):
                            a[temp_counter] = a[temp_counter] - 1
                count = count + 1
            uniqu_ele_cat_values = a.copy()

            encoded_var = [encoded_var[e] for e in range(len(encoded_var)) if e not in index_to_drop_cat_var]

            index_where_to_stop_for_mapped = sum([uniqu_ele_cat_values[i] for i in range(mpfeatures)])
            print("Number of variables mapped after one hot encoding ", index_where_to_stop_for_mapped)
            mapped_features = [encoded_var[i] for i in range(int(index_where_to_stop_for_mapped))]
            possible_feat_to_shuffle = bin_features + [i for i in encoded_var if i not in mapped_features]
            ...
            print("size of total train", len(full_Data))
            ...
            for partition in range(n_p):
                ...
                index_for_mapping_orig_to_rename = [reorder_feat.index(num) + len(mapped_features) + 1 for num in
                ...
                index_for_mapping_orig_to_rename = list(
                    np.arange(1, int(index_where_to_stop_for_mapped) + 1)) + index_for_mapping_orig_to_rename
                index_for_mapping_rename_to_orig = list(
                    np.arange(1, int(index_where_to_stop_for_mapped) + 1)) + index_for_mapping_rename_to_orig
                ...
                reordered_column_names_orig = df_train_preproc.columns
                ...
                Mistakes_X1_tr_sim_Cor, Mistakes_X2_tr_sim_Cor, match_details_x1_sim_cor, match_details_x2_sim_cor, mapp_fea_to_add, mapp_fea_to_add_match,F1_x1_sim_cor, F1_x2_sim_cor = Simple_maximum_sim_viaCorrelation(
                    df_train_preproc.copy(), df_rename_preproc.copy(), index_for_mapping_orig_to_rename,
                    index_for_mapping_rename_to_orig, df_train_preproc.columns, reordered_column_names_r,
                    mapped_features, Cor_from_df, Df_holdout_orig,
                    DF_holdout_r,P_x1[len(mapped_features):-1,len(mapped_features):-1])

                # Mistakes_X1_tr_RG, Mistakes_X2_tr_RG, F1_x1_RG, F1_x2_RG = RadialGAN(df_train_preproc.copy(), df_rename_preproc.copy(),
                #                                                  index_for_mapping_orig_to_rename,
                #                                                  index_for_mapping_rename_to_orig,
                #                                                  df_train_preproc.columns, reordered_column_names_r,
                ...
                Mistakes_X1_tr, Mistakes_X2_tr, match_details_x1, match_details_x2, F1_x1, F1_x2 = Train_cross_AE(
                    ...
                    mapped_features_updated_r, Cor_from_df, Df_holdout_orig, DF_holdout_r,P_x1_updated[len(mapped_features):-1,len(mapped_features):-1])
...
# list_of_number_samples = [1000]
list_of_number_samples = [1000,5000,10000,20000,50000,95000]
#
...
weight_direct = 0.5
weight_cross = 1  # 0 denotes no cross loss, 1 denotes cross loss
weight_cycle = 0.8
...
saving_dir = './ChimericAE_Final_'+str(datetime.date.today()) +  '/ACTFAST_data/Frac_Shuffled_' + str(
    ...
    num_of_hidden_layers) + '/L_dim_' + str(hidden_dim) + "_orthoStatus_" + str(orthogonalization_type) + "_BNStatus_" + str(batchnorm) + "_Varying_sample_size"
...
plt.legend()
plt.savefig(saving_dir + "/F1_Comp_ChimVsRG_X1_tr_Real_data_varying_sample_size_" + str(
...
plt.title(" ACTFAST data ")
plt.legend()
plt.savefig(saving_dir + "/F1_Comp_Chim_X2_tr_Real_data_varying_sample_size_" + str(
    len(list_of_number_samples)) + ".pdf", bbox='tight')
plt.savefig(saving_dir + "/F1_Comp_Chim_X2_tr_Real_data__varying_sample_size_" + str(
    len(list_of_number_samples)) + ".png", bbox='tight')
plt.close()
